...
export class OrcaExecutorWorker {
  ...
  async initialize() {
    ...
    if (!this.apiToken) throw new Error('DEX_ORCA_EXECUTOR_TOKEN is required.');
    ...
    await this.pool.query(`
      CREATE TABLE IF NOT EXISTS sra_dex_executor_requests (
  ...
  status() {
    return {
      service:'SRA Orca Executor Worker',
      contract:'SRA_DEX_EXECUTOR_V1',
      venue:'ORCA_WHIRLPOOLS',
      network:'SOLANA',
      cluster:this.cluster,
      rpcConfigured:Boolean(this.rpcUrl),
      credentialConfigured:Boolean(this.apiToken),
      signerConfigured:Boolean(text(this.env.SOLANA_PAYER_SECRET_KEY)),
      durableIdempotencyConfigured:Boolean(this.pool),
      ready:this.ready,
    };
  }
  ...
  validate(input = {}, idempotencyKey) {
    ...
    return {
      contract:'SRA_DEX_EXECUTOR_V1', action:'CREATE_POOL_AND_SEED_LIQUIDITY', venue:'ORCA_WHIRLPOOLS', network:'SOLANA',
      dexExportId,
      sourceExportPackageId:required(input.sourceExportPackageId,'sourceExportPackageId'),
      projectionId:required(input.projectionId,'projectionId'),
      instrumentId:required(input.instrumentId,'instrumentId'),
      pair:required(input.pair,'pair'),
      baseMintAddress:required(input.baseMintAddress,'baseMintAddress'),
      quoteMintAddress:required(input.quoteMintAddress,'quoteMintAddress'),
      baseLiquidityQuantity:positive(input.baseLiquidityQuantity,'baseLiquidityQuantity'),
      ...
      tickSpacing:integer(input.tickSpacing,'tickSpacing',64),
      liquidityStrategy:text(input.liquidityStrategy || 'FULL_RANGE').toUpperCase(),
      maxSlippageBps:integer(input.maxSlippageBps,'maxSlippageBps',100),
      recordedValueReference:input.recordedValueReference || null,
  ...
  deployment(sdk) {
    return this.cluster === 'mainnet' ? sdk.WhirlpoolDeployment.mainnet : sdk.WhirlpoolDeployment.devnet;
  }
  ...
  async execute(input, idempotencyKey) {
    ...
    try {
      ...
      if (!row) {
        row = (await client.query(`INSERT INTO sra_dex_executor_requests(idempotency_key,request_hash,state) VALUES($1,$2,'STARTED') RETURNING *`, [idempotencyKey,requestHash])).rows[0];
      }

      const sdk = await this.loadSdk();
      const deployment = this.deployment(sdk);
      const base = sdk.address(request.baseMintAddress);
      const quote = sdk.address(request.quoteMintAddress);
      const ordered = sdk.orderMints(base, quote);
      const tokenA = ordered[0];
      const tokenB = ordered[1];
      const baseIsA = String(tokenA) === request.baseMintAddress;
      const initialPrice = baseIsA ? request.initialMarketPrice : 1 / request.initialMarketPrice;
      const [baseDecimals, quoteDecimals] = await Promise.all([
        mintDecimals(this.rpcUrl, request.baseMintAddress),
        mintDecimals(this.rpcUrl, request.quoteMintAddress),
      ]);
      const baseRaw = decimalToBaseUnits(request.baseLiquidityQuantity, baseDecimals);
      const quoteRaw = decimalToBaseUnits(request.quoteLiquidityQuantity, quoteDecimals);
      const tokenMaxA = baseIsA ? baseRaw : quoteRaw;
      const tokenMaxB = baseIsA ? quoteRaw : baseRaw;

      const preparedPool = await sdk.createConcentratedLiquidityPool(tokenA, tokenB, request.tickSpacing, { initialPrice, whirlpoolDeployment:deployment });
      const poolAddress = String(preparedPool.poolAddress);
      if (!row.pool_address) {
        await client.query(`UPDATE sra_dex_executor_requests SET pool_address=$2,state='POOL_PREPARED',updated_at=NOW() WHERE idempotency_key=$1`, [idempotencyKey,poolAddress]);
      ...
      let poolSignature = row.pool_signature;
      const exists = await accountExists(this.rpcUrl, poolAddress);
      if (!exists) {
        poolSignature = String(await preparedPool.callback());
        await client.query(`UPDATE sra_dex_executor_requests SET pool_signature=$2,state='POOL_CREATED',updated_at=NOW() WHERE idempotency_key=$1`, [idempotencyKey,poolSignature]);
      } else if (!isRecovery && !row.pool_signature) {
        throw new Error('The Orca pool already exists and was not created by this executor request.');
      }

      row = (await client.query('SELECT * FROM sra_dex_executor_requests WHERE idempotency_key=$1', [idempotencyKey])).rows[0];
      let liquiditySignature = row.liquidity_signature;
      let positionMint = row.position_mint;
      if (!liquiditySignature) {
        const position = await sdk.openFullRangePosition(sdk.address(poolAddress), { tokenMaxA, tokenMaxB }, { slippageToleranceBps:request.maxSlippageBps, whirlpoolDeployment:deployment });
        positionMint = String(position.positionMint || position.positionAddress || '');
        await client.query(`UPDATE sra_dex_executor_requests SET position_mint=$2,state='LIQUIDITY_PREPARED',updated_at=NOW() WHERE idempotency_key=$1`, [idempotencyKey,positionMint || null]);
        liquiditySignature = String(await position.callback());
        await client.query(`UPDATE sra_dex_executor_requests SET liquidity_signature=$2,state='LIQUIDITY_SEEDED',updated_at=NOW() WHERE idempotency_key=$1`, [idempotencyKey,liquiditySignature]);
      }

      const response = {
        connectorReference:`ORCA:${idempotencyKey}`,
        executionId:idempotencyKey,
        state:'CONFIRMED',
        transactionSignature:liquiditySignature || poolSignature,
        poolAddress,
        positionMint:positionMint || null,
        poolCreationSignature:poolSignature || null,
        liquiditySignature:liquiditySignature || null,
        executedQuantity:request.baseLiquidityQuantity,
        observedMarketPrice:request.initialMarketPrice,
        observedMarketPriceSource:'POOL_INITIALIZATION_INPUT',
        priceReferenceOnly:true,
        recordedValueReference:request.recordedValueReference,
        cluster:this.cluster,
      };
      await client.query(`UPDATE sra_dex_executor_requests SET state='COMPLETED',response_json=$2,last_error=NULL,updated_at=NOW() WHERE idempotency_key=$1`, [idempotencyKey,response]);
      return response;
    } catch (error) {
      await client.query(`UPDATE sra_dex_executor_requests SET last_error=$2,updated_at=NOW() WHERE idempotency_key=$1`, [idempotencyKey, String(error?.message || error)]).catch(()=>{});
      throw error;
    } finally {
      await client.query('SELECT pg_advisory_unlock(hashtext($1))', [idempotencyKey]).catch(()=>{});
      client.release();
    }
  }
