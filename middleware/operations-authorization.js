...
function isParticipantFundingSelfService(req){return (req.method==='POST'&&(req.path==='/api/funding/opportunities'||/^\/api\/funding\/opportunities\/[^/]+\/documents$/.test(req.path)||/^\/api\/funding-verification\/opportunities\/[^/]+\/applicant-information$/.test(req.path)))||(req.method==='GET'&&req.path==='/api/funding-verification/participant-actions');}
...
export function createOperationsAuthorization({accessServiceProvider=defaultAccessService}={}){
  return async function authorizeOperationsRequest(req,res,next){
    ...
    try{
      ...
      const standardSession=standardToken?await service.getSession(standardToken):null;
      const privateAdminSession=privateAdminToken?await service.getSession(privateAdminToken):null;
      const required=requiredRoles(req.path);
      const participantSelfService=isParticipantFundingSelfService(req);
      const usePrivateAdmin=!participantSelfService&&privateAdminSession&&hasRequiredRole(privateAdminSession,required);
      const session=usePrivateAdmin?privateAdminSession:(standardSession||privateAdminSession);
      const source=usePrivateAdmin||(!standardSession&&privateAdminSession)?'PRIVATE_ADMIN_SESSION':session?'SERVER_SESSION':null;
