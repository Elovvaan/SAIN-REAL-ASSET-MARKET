...
function isFinancingExportDocument(path){return /^\/api\/financing-closing\/exports\/[^/]+\/(?:funding-package|ach-settlement-packet)$/.test(path);}
...
function requiresSession(req){return isProductionProtected(req.path)||WRITE_METHODS.has(req.method)||isFinancingAvailabilityLetter(req.path)||isFinancingExportDocument(req.path)||isParticipantFundingSelfService(req);}
...
export function createOperationsAuthorization({accessServiceProvider=defaultAccessService}={}){
  return async function authorizeOperationsRequest(req,res,next){
    ...
    try{
      ...
      if(isParticipantFundingSelfService(req)&&source==='SERVER_SESSION')return next();
