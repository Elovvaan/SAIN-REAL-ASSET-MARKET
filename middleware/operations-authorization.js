...
export function createOperationsAuthorization({accessServiceProvider=defaultAccessService}={}){
  return async function authorizeOperationsRequest(req,res,next){
    ...
    try{
      ...
      const privateAdminAllowed=req.path.startsWith('/api/on-chain')||req.path.startsWith('/api/platform-treasury')||req.path.startsWith('/api/funding')||req.path.startsWith('/api/financing-closing');
