...
export class ProductionReadinessService {
  ...
  async assess() {
    ...
    const checks = [
      ...
      { id:'INTERNAL_SETTLEMENT_OWNERSHIP_GATE', phase:3, status:'PASS', detail:'Ownership recognition requires an existing SRA settlement record and allocation position. No external network response is required for core completion.' },
      { id:'EXTERNAL_ADAPTER_ISOLATION', phase:3, status:'PASS', detail:'Solana, Fedwire, ACH, bank, institution, and partner connectors remain below the export boundary and do not determine core readiness.' },
      ...
      { id:'DEPENDENCY_HEALTH_AND_GRACEFUL_SHUTDOWN', phase:4, status:'PASS', detail:'Database and startup dependency checks are exposed, and SIGTERM/SIGINT perform graceful shutdown. External connector health remains informational.' },
