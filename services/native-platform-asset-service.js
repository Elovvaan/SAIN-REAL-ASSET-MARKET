...
export class NativePlatformAssetService {
  ...
  async ensureInitialOwnership(actorId = 'SRA_PLATFORM_MIGRATION') {
    ...
    const timestamp = now();
    const changes = [];
    if (!position.initialOwnerId || !position.symbol || !position.initialOwnershipRecognitionId || position.assetIdentity !== 'SRA_COIN' || position.fungibility !== 'FUNGIBLE') changes.push({ type: RECORD_TYPES.COIN_POSITION, id: position.coinPositionId, payload: { ...position, symbol: 'SRA', assetIdentity: 'SRA_COIN', assetName: 'SRA Coin', fungibility: 'FUNGIBLE', initialOwnerId: position.initialOwnerId || PLATFORM_OWNER_ID, initialOwnershipRecognitionId: position.initialOwnershipRecognitionId || ownershipRecognitionId, ownershipBasis: position.ownershipBasis || 'SRA_PLATFORM_FORMATION_RECORD', updatedAt: timestamp }, eventType: 'NATIVE_SRA_INITIAL_OWNER_LINKAGE_ENFORCED' });
