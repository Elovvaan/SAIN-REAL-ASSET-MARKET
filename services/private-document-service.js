...
export class PrivateDocumentService {
  ...
  async store({ file, documentType = 'OTHER', uploaderId = null }) {
    ...
    const record = {
      ...
      accessState: 'RESTRICTED', uploaderId, uploadedAt: new Date().toISOString(),
      reviewState: 'SUBMITTED', public: false, storagePath
    };
    this.records.set(id, record);
    if (this.database) {
      await this.database.putDocument(id, record);
      await this.database.audit({ actorId: uploaderId, eventType: 'PRIVATE_DOCUMENT_STORED', objectType: 'PRIVATE_DOCUMENT', objectId: id, payload: { sha256: digest, documentType, size: file.size } });
    }
