...
function providerExecuted(status) {
  return ['COMPLETED','EXECUTED','SETTLED','CONFIRMED','SUCCESS','SUCCEEDED'].includes(String(status || '').toUpperCase());
}
...
export class TreasuryLiveExecutionService {
  ...
  async executeOneDollarAch(input = {}, actorId = 'SRA_PLATFORM_ADMIN') {
    ...
    const executed = providerExecuted(evidence.providerStatus);
