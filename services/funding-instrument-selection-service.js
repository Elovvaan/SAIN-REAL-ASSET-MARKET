...
const TYPES = Object.freeze({
  ...
  INSTRUMENT_ASSESSMENT: 'FUNDING_INSTRUMENT_ASSESSMENT', INSTRUMENT_SELECTION: 'FUNDING_INSTRUMENT_SELECTION', SRA_INSTRUMENT: 'SRA_INSTRUMENT',
});
const INSTRUMENT_FAMILIES = Object.freeze(['TRUE_BILL','COMMERCIAL_PAPER','PARTICIPATION_POSITION','REVENUE_PARTICIPATION_INSTRUMENT','ASSET_BACKED_NOTE','CONSTRUCTION_FUNDING_NOTE','PURCHASE_ORDER_INSTRUMENT','INVOICE_FINANCE_INSTRUMENT','WORKING_CAPITAL_NOTE','EQUIPMENT_FINANCE_INSTRUMENT']);
const MODEL_FAMILY_MAP = Object.freeze({
  PROJECT_FUNDING:['TRUE_BILL','PARTICIPATION_POSITION','COMMERCIAL_PAPER'], PLATFORM_FUNDING:['COMMERCIAL_PAPER','PARTICIPATION_POSITION','TRUE_BILL'],
  CONSTRUCTION_FUNDING:['CONSTRUCTION_FUNDING_NOTE','TRUE_BILL','ASSET_BACKED_NOTE'], REVENUE_PARTICIPATION:['REVENUE_PARTICIPATION_INSTRUMENT','PARTICIPATION_POSITION'],
  ASSET_BACKED_FUNDING:['ASSET_BACKED_NOTE','TRUE_BILL'], PURCHASE_ORDER_FUNDING:['PURCHASE_ORDER_INSTRUMENT','TRUE_BILL'], INVOICE_FUNDING:['INVOICE_FINANCE_INSTRUMENT','TRUE_BILL'], WORKING_CAPITAL:['WORKING_CAPITAL_NOTE','COMMERCIAL_PAPER'], EQUIPMENT_FUNDING:['EQUIPMENT_FINANCE_INSTRUMENT','ASSET_BACKED_NOTE'],
});
...
function ratio(value,basis){return Number.isFinite(value)&&Number.isFinite(basis)&&basis>0?Number((value/basis).toFixed(6)):null;}
...
export class FundingInstrumentSelectionService{
  ...
  assess(requestId){
    ...
    const mapped=MODEL_FAMILY_MAP[request.fundingModel]||[];const candidates=unique([...(request.candidateInstrumentFamilies||[]),...mapped]);
    const assessments=candidates.map((family,index)=>({instrumentFamily:family,score:Math.max(10,100-index*15),reasons:[`Mapped to funding model ${request.fundingModel}.`,request.requiredCharacteristics?.purposeBound?'Supports purpose-bound structuring.':null,request.requiredCharacteristics?.amountBound?'Supports amount-bound structuring.':null,request.requiredCharacteristics?.lifecycleRequired?'Supports lifecycle tracking.':null].filter(Boolean)}));
  ...
  async saveAssessment(requestId,actorId=null){const summary=this.assess(requestId);const record={instrumentAssessmentId:id('FIA'),instrumentSelectionRequestId:requestId,opportunityId:summary.opportunityId,fundingModel:summary.fundingModel,candidates:summary.candidates,recommendedInstrumentFamily:summary.recommendedInstrumentFamily,status:'ASSESSMENT_COMPLETE',assessedBy:actorId,assessedAt:now()};await this.domain.put(TYPES.INSTRUMENT_ASSESSMENT,record.instrumentAssessmentId,record,{actorId,eventType:'FUNDING_INSTRUMENT_ASSESSMENT_COMPLETED'});return record;}
  async selectInstrumentFamily(requestId,input,actorId=null){
    const request=this.getRequest(requestId);if(!request)throw new Error('Instrument selection request was not found.');if(request.status!=='PENDING_INSTRUMENT_SELECTION')throw new Error(`Instrument family cannot be selected from ${request.status}.`);
    const selectedInstrumentFamily=input?.selectedInstrumentFamily;if(!INSTRUMENT_FAMILIES.includes(selectedInstrumentFamily))throw new Error(`Unsupported instrument family: ${selectedInstrumentFamily}`);
    const assessment=await this.saveAssessment(requestId,actorId),candidate=assessment.candidates.find((entry)=>entry.instrumentFamily===selectedInstrumentFamily);if(!candidate)throw new Error('Selected instrument family was not included in the assessment.');
  ...
  async createDraftInstrument(selectionId,input={},actorId=null){
    const selection=this.getSelection(selectionId);if(!selection)throw new Error('Instrument selection was not found.');if(selection.status!=='SELECTED')throw new Error(`Draft instrument cannot be created from ${selection.status}.`);
    const request=this.getRequest(selection.instrumentSelectionRequestId),opportunity=this.domain.get(TYPES.OPPORTUNITY,selection.opportunityId);if(!request||!opportunity)throw new Error('Required funding records were not found.');
    ...
    await this.domain.lifecycle({objectType:TYPES.OPPORTUNITY,objectId:opportunity.opportunityId,eventType:'FUNDING_INSTRUMENT_DRAFT_CREATED',actorId,payload:{instrumentId:instrument.instrumentId,instrumentFamily:instrument.instrumentFamily,issuanceStatus:instrument.issuanceStatus,canonicalVerifiedValueRecordId:instrument.canonicalVerifiedValueRecordId,requestedAmount:instrument.requestedAmount,recognizedReferenceValue:instrument.recognizedReferenceValue,faceValue:instrument.faceValue,faceValueBasis:instrument.faceValueBasis,faceValueToRecognizedRatio:instrument.faceValueToRecognizedRatio}});
    return instrument;
  }
}
