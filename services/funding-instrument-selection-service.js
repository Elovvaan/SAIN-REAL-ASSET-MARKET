...
const INSTRUMENT_FAMILIES = Object.freeze([
  ...
  'REVENUE_PARTICIPATION_INSTRUMENT',
  'ASSET_BACKED_NOTE',
  'CONSTRUCTION_FUNDING_NOTE',
  'PURCHASE_ORDER_INSTRUMENT',
  'INVOICE_FINANCE_INSTRUMENT',
  'WORKING_CAPITAL_NOTE',
  'EQUIPMENT_FINANCE_INSTRUMENT',
]);
...
export class FundingInstrumentSelectionService {
  ...
  status() {
    return {
      service: 'SRA Funding Engine Phase 5',
      purpose: 'INSTRUMENT_FAMILY_ASSESSMENT_AND_DRAFT_CREATION',
      assessments: this.domain.list(TYPES.INSTRUMENT_ASSESSMENT).length,
      selections: this.domain.list(TYPES.INSTRUMENT_SELECTION).length,
      draftInstruments: this.domain.list(TYPES.SRA_INSTRUMENT).filter((record) => record.state === 'DRAFT').length,
    };
  }

  getRequest(requestId) {
    return this.domain.get(TYPES.INSTRUMENT_REQUEST, requestId);
  }

  listAssessments(requestId = null) {
    return this.domain.list(TYPES.INSTRUMENT_ASSESSMENT).filter((record) => !requestId || record.instrumentSelectionRequestId === requestId);
  }

  getSelection(selectionId) {
    return this.domain.get(TYPES.INSTRUMENT_SELECTION, selectionId);
  }

  assess(requestId) {
    const request = this.getRequest(requestId);
    if (!request) throw new Error('Instrument selection request was not found.');

    const mapped = MODEL_FAMILY_MAP[request.fundingModel] || [];
    const candidates = unique([...(request.candidateInstrumentFamilies || []), ...mapped]);
    const assessments = candidates.map((family, index) => ({
      instrumentFamily: family,
      score: Math.max(10, 100 - (index * 15)),
      reasons: [
    ...
    return {
      instrumentSelectionRequestId: requestId,
      opportunityId: request.opportunityId,
      fundingModel: request.fundingModel,
      candidates: assessments,
      recommendedInstrumentFamily: assessments[0]?.instrumentFamily || null,
      note: 'Assessment identifies candidate instrument families. No instrument is issued in this phase.',
    };
  }
  ...
  async saveAssessment(requestId, actorId = null) {
    ...
    const record = {
      instrumentAssessmentId: id('FIA'),
      instrumentSelectionRequestId: requestId,
      opportunityId: summary.opportunityId,
      fundingModel: summary.fundingModel,
      candidates: summary.candidates,
      recommendedInstrumentFamily: summary.recommendedInstrumentFamily,
  ...
  async selectInstrumentFamily(requestId, input, actorId = null) {
    ...
    const assessment = await this.saveAssessment(requestId, actorId);
    const candidate = assessment.candidates.find((entry) => entry.instrumentFamily === selectedInstrumentFamily);
    if (!candidate) throw new Error('Selected instrument family was not included in the assessment.');

    const selection = {
      instrumentSelectionId: input.instrumentSelectionId || id('FIS'),
      instrumentSelectionRequestId: requestId,
      opportunityId: request.opportunityId,
      fundingModel: request.fundingModel,
      selectedInstrumentFamily,
      assessedScore: candidate.score,
      assessedReasons: candidate.reasons,
      recommendedInstrumentFamily: assessment.recommendedInstrumentFamily,
    ...
    await this.domain.put(TYPES.INSTRUMENT_REQUEST, requestId, {
      ...request,
      status: 'INSTRUMENT_FAMILY_SELECTED',
      selectedInstrumentType: selectedInstrumentFamily,
      instrumentAssessmentId: assessment.instrumentAssessmentId,
      instrumentSelectionId: selection.instrumentSelectionId,
      updatedAt: now(),
    }, { actorId, eventType: 'FUNDING_INSTRUMENT_SELECTION_REQUEST_UPDATED' });
  ...
  async createDraftInstrument(selectionId, input = {}, actorId = null) {
    ...
    if (!selection) throw new Error('Instrument selection was not found.');
    if (selection.status !== 'SELECTED') throw new Error(`Draft instrument cannot be created from ${selection.status}.`);

    const request = this.getRequest(selection.instrumentSelectionRequestId);
    ...
    const existing = this.domain.list(TYPES.SRA_INSTRUMENT).find((record) => record.instrumentSelectionId === selectionId && record.state === 'DRAFT');
    if (existing) return existing;

    const instrument = {
      instrumentId: input.instrumentId || id('SRAI'),
      instrumentFamily: selection.selectedInstrumentFamily,
      instrumentType: selection.selectedInstrumentFamily,
      fundingModel: selection.fundingModel,
      opportunityId: selection.opportunityId,
      instrumentSelectionRequestId: selection.instrumentSelectionRequestId,
      instrumentSelectionId: selectionId,
      ...
      faceValue: Number(input.faceValue ?? request.requestedAmount),
      currency: input.currency || request.currency,
      denomination: input.denomination || null,
      maturityDate: input.maturityDate || null,
      transferabilityStatus: input.transferabilityStatus || 'RESTRICTED',
      settlementRule: input.settlementRule || null,
      governingDocumentId: input.governingDocumentId || null,
      terms: { ...(selection.terms || {}), ...(input.terms || {}) },
      restrictions: unique([...(selection.restrictions || []), ...(input.restrictions || [])]),
      state: 'DRAFT',
      status: 'DRAFT',
    ...
    await this.domain.put(TYPES.INSTRUMENT_REQUEST, request.instrumentSelectionRequestId, {
      ...request,
      status: 'DRAFT_INSTRUMENT_CREATED',
      selectedInstrumentId: instrument.instrumentId,
      completedAt: now(),
    }, { actorId, eventType: 'FUNDING_INSTRUMENT_SELECTION_COMPLETED' });
    await this.domain.put(TYPES.OPPORTUNITY, opportunity.opportunityId, {
      ...opportunity,
      status: 'INSTRUMENT_DRAFTED',
      fundingPhase: 'INSTRUMENT_DRAFT_REVIEW',
      selectedInstrumentId: instrument.instrumentId,
      updatedAt: now(),
      history: [...(opportunity.history || []), { from: opportunity.status, to: 'INSTRUMENT_DRAFTED', at: now(), actorId, note: selection.selectedInstrumentFamily }],
    }, { actorId, eventType: 'FUNDING_OPPORTUNITY_INSTRUMENT_DRAFTED' });

    await this.domain.lifecycle({
      objectType: TYPES.OPPORTUNITY,
      objectId: opportunity.opportunityId,
      eventType: 'FUNDING_INSTRUMENT_DRAFT_CREATED',
      actorId,
      payload: { instrumentId: instrument.instrumentId, instrumentFamily: instrument.instrumentFamily, issuanceStatus: instrument.issuanceStatus },
    });
