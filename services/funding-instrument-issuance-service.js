...
export class FundingInstrumentIssuanceService {
  ...
  assessRequest(requestId) {
    ...
    return { issuanceRequestId: requestId, instrumentId: instrument.instrumentId, checks, blockers, readyForAuthorization: blockers.length === 0, valueReference: { mode: vvr.mode, canonicalVerifiedValueRecordId: instrument.canonicalVerifiedValueRecordId || null, determinationId: vvr.record?.determinationId || null, snapshotId: vvr.record?.snapshotId || null } };
  }
  ...
  async decide(reviewId, input, actorId = null) {
    ...
    if (input.decision === 'AUTHORIZED') {
      authorization = { issuanceAuthorizationId: id('FIAU'), issuanceRequestId: request.issuanceRequestId, issuanceReviewId: reviewId, instrumentId: request.instrumentId, opportunityId: request.opportunityId, authorizedFaceValue: request.faceValue, currency: request.currency, canonicalVerifiedValueRecordId: assessment.valueReference.canonicalVerifiedValueRecordId, valueReferenceMode: assessment.valueReference.mode, authorizedIssueDate: input.authorizedIssueDate || request.requestedIssueDate || null, authorizedMaturityDate: input.authorizedMaturityDate || request.requestedMaturityDate || null, conditions: unique([...(request.issuanceConditions || []), ...(input.conditions || [])]), status: 'AUTHORIZED', authorizedBy: actorId, authorizedAt: decidedAt, consumedAt: null, issuanceTransactionId: null };
  ...
  async issue(authorizationId, input = {}, actorId = null) {
    ...
    const transaction = { transactionId: input.transactionId || id('SRATX'), transactionType: 'INSTRUMENT_ISSUANCE', instrumentId: instrument.instrumentId, opportunityId: instrument.opportunityId, issuerParticipantId: instrument.issuerParticipantId, issuanceAuthorizationId: authorizationId, canonicalVerifiedValueRecordId: instrument.canonicalVerifiedValueRecordId || null, referencedDeterminationId: vvr.record?.determinationId || null, referencedSnapshotId: vvr.record?.snapshotId || null, amount: authorization.authorizedFaceValue, currency: authorization.currency, issueDate: authorization.authorizedIssueDate || issuedAt, maturityDate: authorization.authorizedMaturityDate || instrument.maturityDate || null, governingDocumentId: instrument.governingDocumentId, settlementRule: instrument.settlementRule, state: 'RECORDED', status: 'RECORDED', recordedBy: actorId, recordedAt: issuedAt };
    const issuedInstrument = { ...instrument, state: 'ISSUED', status: 'ACTIVE', issuanceStatus: 'ISSUED', issuanceAuthorizationId: authorizationId, issuanceTransactionId: transaction.transactionId, valueReferenceArchitecture: vvr.mode === 'CANONICAL_VVR_REFERENCE' ? 'CANONICAL_VVR_REFERENCE' : instrument.valueReferenceArchitecture || 'LEGACY_VERIFIED_RECORD_REFERENCE', issueDate: transaction.issueDate, maturityDate: transaction.maturityDate, issuedBy: actorId, issuedAt, updatedAt: issuedAt };
