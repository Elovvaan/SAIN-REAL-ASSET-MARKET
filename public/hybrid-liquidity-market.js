(() => {
  ...
  function card(market) {
    ...
    const referenceMarkup = reference
      ? `<div><span>Latest reference</span><strong>${money.format(Number(reference.referenceValue || 0))}</strong><small>${esc(reference.quoteCurrency || 'USD')} · market reference</small></div>`
    ...
    const executionMarkup = spotAvailable
      ...
      : `<div><span>Governed spot access</span><strong>REFERENCE ONLY</strong><small>${market.mode === 'SPOT' ? 'Next: activate marketplace listing.' : 'View market reference.'}</small></div>`;
    ...
    root.querySelectorAll('[data-open-hybrid-order]').forEach((button) => button.addEventListener('click', () => {
      ...
      panel.querySelector('[data-hybrid-review-order]').addEventListener('click', async () => {
        ...
        try {
          ...
          const referenceCopy = preview.hybridSpot?.referenceValue != null ? ` Hybrid reference: ${money.format(Number(preview.hybridSpot.referenceValue))}.` : '';
