    ...
    try {
      ...
      detailRoot.innerHTML = `
        <div class="instrument-grid">
          ...
          <section class="instrument-card"><p class="eyebrow">INSTRUMENT STATE</p><strong>${esc(instrument?.instrumentId || 'No draft instrument')}</strong><p>${esc(instrument?.status || 'Not created')} · ${esc(instrument?.issuanceStatus || '')}</p><p>${esc(instrumentSelection?.selectedInstrumentFamily || instrumentRequest?.fundingModel || '')}</p><small>${esc(String(instrument?.approvedTransactionStructure || instrumentRequest?.approvedTransactionStructure || instrumentRequest?.proposedTransactionStructure || 'TRANSACTION_STRUCTURE_PENDING').replaceAll('_', ' '))}</small></section>
        </div>
        ...
        ${authorization ? `<section class="instrument-card" style="margin-top:12px"><p class="eyebrow">AUTHORIZED ISSUANCE</p><strong>${esc(authorization.issuanceAuthorizationId)}</strong><p>${esc(authorization.status)} · ${money.format(Number(authorization.authorizedFaceValue || 0))}</p>${instrument?.issuanceStatus === 'ISSUED' ? `<p>Issued instrument presentation: Check-stock layout</p><div class="instrument-actions"><a class="primary-button" href="/api/funding-instrument-issuance/instruments/${encodeURIComponent(instrument.instrumentId)}/document" target="_blank" rel="noopener">Digital instrument PDF</a><a class="secondary-button" href="/api/funding-instrument-issuance/instruments/${encodeURIComponent(instrument.instrumentId)}/document?version=print" target="_blank" rel="noopener">Check-stock print PDF</a></div>` : `<div class="instrument-actions"><button class="primary-button" data-iaction="issue-instrument">Issue instrument</button></div>`}</section>` : ''}
