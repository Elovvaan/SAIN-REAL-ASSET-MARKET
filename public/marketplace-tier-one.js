(() => {
  ...
  const openingAssets = [
    { type: 'DIGITAL FINANCIAL ASSET', title: 'SRA Coin Position', description: 'A position records the source, obligation, documentation, ownership, and quantity supporting the same fungible SRA Coin.', action: 'coin-order' },
  ...
  const formationSteps = [
    ['1', 'Connect', 'An account, market, opportunity, customer asset, or other source enters its existing SRA workflow.'], ['2', 'Observe', 'SRA records the source information and evidence.'], ['3', 'Recognize', 'SAIN recognizes the value, rights, obligations, ownership, and documentation.'], ['4', 'Record', 'The recognized relationship becomes an SRA Financial Record.'], ['5', 'Position', 'An SRA Coin Position records the source and obligation supporting its quantity.'], ['6', 'Create', 'The position creates the same fungible SRA Coin regardless of which workflow produced it.'], ['7', 'Transfer', 'SRA Coin keeps the same identity and behavior when it moves between holders.'], ['8', 'Settle', 'Settlement follows the obligation and documentation attached to its originating position.']
  ];
  ...
  async function loadMarketFlows(root) {
    const target = root?.querySelector('[data-sra-market-flows]');
    if (!target) return;
    try {
      const response = await fetch('/api/connectors/coinbase-public/status');
      const status = await response.json();
      if (!response.ok) throw new Error('Market flow unavailable');
      const flows = status.transactionAssetPipeline?.flowPositions || [];
      target.innerHTML = flows.length ? flows.map((flow) => `<article class="tier-one-native-asset"><span class="badge open">${pEsc(flow.marketState || 'LIVE')}</span><h3>${pEsc(flow.productId)} SRA Market Flow</h3><p>${Number(flow.tradeCount || 0).toLocaleString()} source trades summarized into ${Number(flow.batchCount || 0).toLocaleString()} intake records.</p><div class="tier-one-opportunity-metrics"><span><small>Source quantity</small><strong>${Number(flow.nativeQuantity || 0).toLocaleString()} ${pEsc(flow.nativeUnit || '')}</strong></span><span><small>SRA represented</small><strong>${Number(flow.representedSra || 0).toLocaleString()}</strong></span><span><small>Destination</small><strong>SRA Market</strong></span><span><small>Last price</small><strong>${Number(flow.lastPrice || 0).toLocaleString()}</strong></span></div></article>`).join('') : '<div class="empty-view"><p>The first live market flow will appear after the current intake window closes.</p></div>';
    } catch { target.innerHTML = '<div class="empty-view"><p>Live market flow is refreshing.</p></div>'; }
  }
  ...
  renderSignedInMarketplace = function renderSignedInMarketplaceTierOne() {
    ...
    document.querySelector('#explain-formation-path')?.addEventListener('click', () => askSain('Explain how Coinbase activity, opportunities, customer assets, and other eligible sources create Coin Positions in the same fungible SRA Coin.'));
    document.querySelector('#explain-sra-coin')?.addEventListener('click', () => askSain('Explain how every SRA Coin has the same identity and behavior while its Coin Position preserves the source obligation and documentation.'));
