(() => {
  ...
  async function openCoinOrderWorkflow() {
    ...
    if (balance <= 0) blockers.push('Your Asset Vault does not currently contain available recorded funds.');
    ...
    root.innerHTML = `<section class="tier-one-live-market coin-order-workflow" aria-label="Coin order workflow"><div class="tier-one-section-head"><div><p class="eyebrow">COIN ORDER WORKFLOW</p><h2>Review SRA Coin order</h2></div><button class="secondary-button" data-return-marketplace>Return to marketplace</button></div><div class="tier-one-market-summary"><div><span>Selected asset</span><strong>${pEsc(positionName)}</strong></div><div><span>Current Verified Value</span><strong>${money.format(verifiedValue)}</strong></div><div><span>Available Asset Vault balance</span><strong>${money.format(balance)}</strong></div></div><div class="tier-one-native-asset-grid"><article class="tier-one-native-asset"><span class="badge open">STEP 1</span><h3>Select Coin Position</h3><p>${selected ? `Position ${pEsc(selected.coinPositionId || selected.id || 'recorded')} is available for review.` : 'No active position is available to select yet.'}</p></article><article class="tier-one-native-asset"><span class="badge open">STEP 2</span><h3>Enter order amount</h3><p>Quantity and consideration controls activate only when a published offering and price record are connected.</p></article><article class="tier-one-native-asset"><span class="badge open">STEP 3</span><h3>Review account funding</h3><p>The order must use recorded available funds from this Universal Account’s Asset Vault.</p></article><article class="tier-one-native-asset"><span class="badge open">STEP 4</span><h3>Submit and settle</h3><p>Submission must create an authorized order, ownership event, transaction record, and settlement instruction.</p></article></div><section class="coin-order-blockers"><p class="eyebrow">CURRENT CONNECTION STATE</p><h3>Order cannot be submitted until:</h3><ul>${blockers.map((item) => `<li>${pEsc(item)}</li>`).join('')}</ul></section><div class="tier-one-coin-actions"><button class="secondary-button" id="explain-current-coin-order">Ask SAIN to explain these records</button><button class="primary-button" disabled aria-disabled="true">Submit order</button></div></section>`;
    root.querySelector('[data-return-marketplace]')?.addEventListener('click', renderSignedInMarketplace);
