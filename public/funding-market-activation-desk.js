(() => {
  ...
  async function loadOpportunity(root) {
    ...
    try {
      ...
      detailRoot.querySelector('[data-maction="create-preparation"]')?.addEventListener('click', () => act(
        () => request(`/api/funding-marketplace/instruments/${encodeURIComponent(instrument.instrumentId)}/preparations`, { method: 'POST', body: JSON.stringify({ listingTitle: detailRoot.querySelector('#ma-title').value, publicSummary: detailRoot.querySelector('#ma-summary').value, targetAmount: Number(detailRoot.querySelector('#ma-target').value), listingPrice: Number(detailRoot.querySelector('#ma-price').value), minimumParticipation: Number(detailRoot.querySelector('#ma-min').value), maximumParticipation: Number(detailRoot.querySelector('#ma-max').value), openAt: detailRoot.querySelector('#ma-open').value || null, closeAt: detailRoot.querySelector('#ma-close').value || null, settlementRoute: detailRoot.querySelector('#ma-settlement-route').value || null, marketAccessRules: detailRoot.querySelector('#ma-access-rules').value || null, transactionRoute: detailRoot.querySelector('#ma-transaction-route').value || null, riskDisclosures: detailRoot.querySelector('#ma-disclosures').value || null }) }), 'Marketplace preparation created.'
      ));
      detailRoot.querySelector('[data-maction="review-preparation"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace/preparations/${encodeURIComponent(preparation.preparationId)}/review`, { method: 'POST', body: JSON.stringify({ decision: 'APPROVED', rationale: 'Marketplace preparation is complete and ready for listing creation.' }) }), 'Marketplace preparation reviewed.'));
      detailRoot.querySelector('[data-maction="create-listing"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace/preparations/${encodeURIComponent(preparation.preparationId)}/listing`, { method: 'POST', body: '{}' }), 'Prepared listing created.'));
      ...
      detailRoot.querySelector('[data-maction="authorize-publication"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace-publication/reviews/${encodeURIComponent(publicationReview.reviewId)}/decision`, { method: 'POST', body: JSON.stringify({ decision: 'AUTHORIZED', rationale: 'Listing satisfies publication controls.' }) }), 'Publication authorized.'));
      detailRoot.querySelector('[data-maction="publication-changes"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace-publication/reviews/${encodeURIComponent(publicationReview.reviewId)}/decision`, { method: 'POST', body: JSON.stringify({ decision: 'CHANGES_REQUIRED', rationale: 'Listing requires publication corrections.' }) }), 'Listing returned for changes.'));
      detailRoot.querySelector('[data-maction="publish-listing"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace-publication/authorizations/${encodeURIComponent(publicationAuthorization.publicationAuthorizationId)}/publish`, { method: 'POST', body: '{}' }), 'Listing published to the marketplace.'));
      ...
      detailRoot.querySelector('[data-maction="approve-allocation"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace-allocation/reviews/${encodeURIComponent(allocationReview.reviewId)}/decision`, { method: 'POST', body: JSON.stringify({ decision: 'APPROVED', rationale: 'Confirmed commitments are ready for position creation.' }) }), 'Allocations approved.'));
      detailRoot.querySelector('[data-maction="allocation-changes"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace-allocation/reviews/${encodeURIComponent(allocationReview.reviewId)}/decision`, { method: 'POST', body: JSON.stringify({ decision: 'CHANGES_REQUIRED', rationale: 'Allocation requires correction.' }) }), 'Allocation returned for changes.'));
      detailRoot.querySelector('[data-maction="create-positions"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace-allocation/reviews/${encodeURIComponent(allocationReview.reviewId)}/positions`, { method: 'POST', body: '{}' }), 'Ownership positions created.'));
      ...
      detailRoot.querySelector('[data-maction="authorize-settlement"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace-settlement/reviews/${encodeURIComponent(settlementReview.reviewId)}/decision`, { method: 'POST', body: JSON.stringify({ decision: 'AUTHORIZED', rationale: 'Settlement preparation satisfies the ownership recognition controls.' }) }), 'Settlement authorized.'));
      detailRoot.querySelector('[data-maction="settlement-changes"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace-settlement/reviews/${encodeURIComponent(settlementReview.reviewId)}/decision`, { method: 'POST', body: JSON.stringify({ decision: 'CHANGES_REQUIRED', rationale: 'Settlement preparation requires correction.' }) }), 'Settlement returned for changes.'));
      detailRoot.querySelector('[data-maction="settle-position"]')?.addEventListener('click', () => act(() => request(`/api/funding-marketplace-settlement/authorizations/${encodeURIComponent(settlementAuthorization.settlementAuthorizationId)}/settle`, { method: 'POST', body: '{}' }), 'Position settled and ownership recognized.'));
    } catch (error) {
  ...
  async function mount(fundingRoot) {
    ...
    try {
      ...
      section.className = 'market-activation-desk';
      section.id = 'funding-market-activation-desk';
      section.innerHTML = `<strong>Marketplace activation desk could not load.</strong><p>${esc(error.message)}</p>`;
      fundingRoot.append(section);
    }
  }
