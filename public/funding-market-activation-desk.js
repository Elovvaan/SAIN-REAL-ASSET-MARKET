    ...
    try {
      ...
      detailRoot.innerHTML = `
        <div class="market-activation-grid">
          ...
          <section class="market-activation-card"><p class="eyebrow">FUNDED POSITION</p><strong>${esc(financedPosition?.positionId || 'Pending')}</strong><p>${esc(financedPosition?.positionStatus || 'PENDING')} · ${esc(financedPosition?.distributionStatus || 'PENDING')}</p><p>${financedPosition ? `${money.format(Number(financedPosition.retainedAmount || 0))} retained · ${money.format(Number(financedPosition.offeredAmount || financedPosition.availableAmount || 0))} offered/available` : 'Next: complete financing.'}</p></section>
        </div>
        ${!financedPosition ? `<section class="market-activation-card" style="margin-top:12px"><p class="eyebrow">FINANCING</p><strong>Pending</strong><p>Next: complete financing and settlement.</p></section>` : ''}
        ${financedPosition && financedPosition.distributionStatus === 'RETAINED' ? `<section class="market-activation-card" style="margin-top:12px"><p class="eyebrow">POSITION DISTRIBUTION</p><strong>Position retained</strong><p>Choose an amount to make available.</p><div class="market-activation-grid"><input id="ma-offered-amount" type="number" min="0.01" step="0.01" max="${Number(financedPosition.currentPrincipal || 0)}" placeholder="Amount of position to make available"><textarea id="ma-transfer-restrictions" placeholder="Transfer restrictions, one per line"></textarea></div><div class="market-activation-actions"><button class="primary-button" data-maction="make-position-available">Make Position Available</button></div></section>` : ''}
