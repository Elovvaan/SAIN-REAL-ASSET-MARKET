(() => {
  ...
  function style() {
    ...
    node.textContent = `
      .funding-ops{display:grid;gap:16px}.funding-ops-hero,.funding-ops-panel,.funding-detail{padding:20px;border:1px solid rgba(255,255,255,.12);border-radius:18px;background:rgba(255,255,255,.025)}.funding-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:16px}.funding-metric{padding:14px;border-radius:13px;background:rgba(255,255,255,.04)}.funding-metric strong{display:block;font-size:21px}.funding-metric span{font-size:12px;opacity:.7}.funding-ops-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.funding-phase-card{padding:15px;border:1px solid rgba(255,255,255,.1);border-radius:14px;background:rgba(255,255,255,.025)}.funding-phase-head,.funding-panel-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.funding-phase-card small{display:block;opacity:.7;margin-top:8px}.funding-phase-state{font-size:11px;padding:5px 8px;border-radius:999px;background:rgba(45,190,120,.15);color:#7de0a9}.funding-ops-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}.funding-ops-list{display:grid;gap:10px;margin-top:12px}.funding-ops-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;padding:13px;border-radius:12px;background:rgba(255,255,255,.035);cursor:pointer}.funding-ops-row:hover{background:rgba(255,255,255,.06)}.funding-ops-row span{display:block;opacity:.72;font-size:12px;margin-top:4px}.funding-next{font-size:12px;text-align:right}.funding-intake-modal,.funding-detail{display:none}.funding-intake-modal.open,.funding-detail.open{display:block}.funding-intake-modal{padding:18px;border:1px solid rgba(215,166,42,.35);border-radius:16px;background:rgba(215,166,42,.06)}.funding-intake-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:14px}.funding-intake-grid input,.funding-intake-grid select,.funding-intake-grid textarea{width:100%;box-sizing:border-box;padding:11px;border:1px solid rgba(255,255,255,.15);border-radius:10px;background:#101010;color:#fff}.funding-intake-grid textarea{grid-column:1/-1;min-height:86px}.funding-intake-result{margin-top:10px;font-size:13px}.funding-detail-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:14px}.funding-detail-card{padding:14px;border-radius:13px;background:rgba(255,255,255,.04)}.funding-detail-card span{display:block;font-size:12px;opacity:.7;margin-top:5px}.funding-evidence-list{display:grid;gap:7px;margin-top:10px}.funding-evidence-item{padding:10px;border-radius:10px;background:rgba(255,255,255,.035);font-size:12px}.funding-ops-empty{padding:16px;opacity:.7}@media(max-width:800px){.funding-ops-grid,.funding-metrics,.funding-intake-grid,.funding-detail-grid{grid-template-columns:1fr}.funding-ops-row{grid-template-columns:1fr}.funding-next{text-align:left}}`;
  ...
  function detailShell() {
    return '<section class="funding-detail" id="funding-detail"></section>';
  }
  ...
  async function openDetail(root, opportunityId) {
    ...
    try {
      ...
      panel.querySelector('[data-action="complete-intake"]')?.addEventListener('click', async () => {
        ...
        try { await request(`/api/funding/opportunities/${encodeURIComponent(opportunityId)}/complete-intake`, { method: 'POST', body: '{}' }); result.textContent = 'Intake completed. Opportunity is ready for verification.'; setTimeout(() => render(root), 700); }
        catch (error) { result.textContent = error.message; }
      });
      panel.querySelector('[data-action="create-verification"]')?.addEventListener('click', async () => {
        const result = panel.querySelector('#funding-detail-result');
        try { const created = await request(`/api/funding/opportunities/${encodeURIComponent(opportunityId)}/verification-requests`, { method: 'POST', body: '{}' }); result.textContent = `Verification request ${created.verificationRequestId} created.`; setTimeout(() => render(root), 700); }
        catch (error) { result.textContent = error.message; }
      });
    } catch (error) {
    ...
    try {
      ...
      root.querySelector('#funding-opportunity-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          result.innerHTML = `<strong>Created ${esc(record.opportunityId)}</strong> · ${esc(record.status)}.`;
