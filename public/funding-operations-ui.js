(() => {
  ...
  const OPPORTUNITY_TRANSACTION_STRUCTURES = Object.freeze({
    ...
    HOME_EQUITY: ['SECURED_INSTRUMENT'],
  ...
  function homeEquityDetail(record) {
    if (record.opportunityType !== 'HOME_EQUITY') return '';
    const equity = record.homeEquityFunding || {};
    return `<section class="funding-ops-panel"><p class="eyebrow">SRA HOME EQUITY FUNDING TRANSACTION</p><div class="funding-detail-grid"><div class="funding-detail-card"><strong>${esc(equity.property?.address || 'Not entered')}</strong><span>Secured property</span></div><div class="funding-detail-card"><strong>${money.format(Number(equity.verifiedEquity || 0))}</strong><span>Verified equity</span></div><div class="funding-detail-card"><strong>${money.format(Number(equity.maximumPrincipalAdvance || 0))}</strong><span>Maximum principal advance</span></div><div class="funding-detail-card"><strong>${Number(equity.projectedCombinedLtvPercent || 0).toFixed(2)}%</strong><span>Projected combined LTV</span></div><div class="funding-detail-card"><strong>${money.format(Number(equity.fixedReturnAmount || 0))}</strong><span>Fixed return</span></div><div class="funding-detail-card"><strong>${money.format(Number(equity.totalRepaymentObligation || 0))}</strong><span>Total repayment obligation</span></div><div class="funding-detail-card"><strong>${Number(equity.repaymentSupport?.paymentCoverageRatio || 0).toFixed(2)}×</strong><span>Verified payment coverage</span></div><div class="funding-detail-card"><strong>${esc(equity.settlementAssetCode || equity.settlementAsset || '')}</strong><span>Settlement asset · ${esc(equity.settlementNetwork || '')}</span></div><div class="funding-detail-card"><strong>${esc(equity.repaymentDenomination || '')}</strong><span>Repayment denomination · ${esc(equity.termMonths || '')} months</span></div></div></section>`;
  }
  ...
  async function render(root) {
    ...
    const homeEquityIntake = root.querySelector('#home-equity-intake');
    ...
    typeSelect?.addEventListener('change', () => {
      ...
      homeEquityIntake?.classList.toggle('open', typeSelect.value === 'HOME_EQUITY');
    ...
    root.querySelector('#funding-opportunity-form')?.addEventListener('submit', async (event) => {
      ...
      if (values.opportunityType === 'HOME_EQUITY') payload.homeEquityFunding = homeEquityPayload(formData);
      try {
        ...
        window.dispatchEvent(new CustomEvent('sra:funding-opportunity-created', { detail: { opportunityId: record.opportunityId, root } }));
        await openDetail(root, record.opportunityId);
      } catch (error) {
        if (result) result.textContent = error.message;
      } finally {
        delete form.dataset.submitting;
