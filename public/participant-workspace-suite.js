(() => {
  ...
  function vaultActivityRows(transactions = []) {
    if (!transactions.length) return '<div class="transaction-empty"><strong>No participant-linked activity yet.</strong><span>Completed account activity will appear here.</span></div>';
    ...
    try {
      ...
      root.innerHTML = `<section class="participant-journey">
        ...
        <section class="participant-journey-section"><h3>Available platform services</h3><div class="product-grid">
          ...
          ${card('SRA Coin', 'View your linked SRA and network representation.', 'assets')}
        </div></section>
      </section>`;
      bindSuiteLinks(root);
    } catch (error) {
  ...
  async function renderSraCoin(root) {
    ...
    try {
      ...
      root.innerHTML = `<section class="participant-journey"><section class="participant-journey-section"><div class="transaction-section-title"><div><p class="eyebrow">SRA / USD REPRESENTATION</p><h2>SRA Coin</h2><p>View your SRA position and current network totals.</p></div><span class="badge open">1 SRA = 1 USD</span></div><div class="participant-home-summary"><article><span>Network represented SRA</span><strong>${qty(network.representedSra)} SRA</strong></article><article><span>Network recognized USD basis</span><strong>${moneyCents.format(network.recognizedUsd)}</strong></article><article><span>Network Coin Positions</span><strong>${network.totalPositionCount}</strong></article><article><span>Your linked position slices</span><strong>${data.coin.participant.length}</strong></article><article><span>Your available SRA</span><strong>${qty(participantSra)} SRA</strong></article></div><div class="journey-list"><div class="journey-row"><strong>Representation coverage ${network.coveragePct.toFixed(1)}%</strong><span>${network.rootPositionCount - network.missingBasis} of ${network.rootPositionCount} root positions include a visible USD basis.</span></div><div class="journey-row"><strong>Position lineage</strong><span>${network.rootPositionCount} root positions · ${network.derivativePositionCount} derivative or segmented slices.</span></div><div class="journey-row"><strong>Available balance</strong><span>Your available SRA is calculated from spendable position quantities.</span></div><div class="journey-row"><strong>Source mix</strong><span>${esc(sources)}</span></div></div></section></section>`;
    } catch (error) {
      if (activeView !== 'assets') return;
