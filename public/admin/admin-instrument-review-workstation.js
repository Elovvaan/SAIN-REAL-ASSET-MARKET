(() => {
  ...
  function pendingTab(workspace) { return String(workspace?.dataset?.activeTab || '') === 'Pending Review'; }
  function nextVersion(workspace) { const version = (renderVersions.get(workspace) || 0) + 1; renderVersions.set(workspace, version); return version; }
  function ownsRender(workspace, version) { return renderVersions.get(workspace) === version && active(workspace) && pendingTab(workspace); }
  ...
  function clear(workspace) { workspace?.querySelector('[data-instrument-review-workstation]')?.remove(); }
  function amountOf(instrument) { return instrument?.denomination?.principalQuantity ?? instrument?.authorizedSupply ?? instrument?.authorizedAmount ?? instrument?.faceValueUsd ?? instrument?.quantity ?? null; }
  ...
  function card(instrument) {
    ...
    return `<article class="admin-record-card" data-review-instrument="${esc(instrumentId)}" style="margin:0"><header><strong>${esc(instrumentId)}</strong><em>${esc(state)}</em></header><div class="admin-record-grid"><div><span>Source Coin Position</span><strong>${esc(coinPositionId)}</strong></div><div><span>Instrument Type</span><strong>${esc(instrument?.instrumentType || 'SRA_VALUE_INSTRUMENT')}</strong></div><div><span>Purpose</span><strong>${esc(purpose)}</strong></div><div><span>Principal / Representation</span><strong>${esc(amount ?? '—')} ${esc(instrument?.denomination?.symbol || '')}</strong></div></div><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button type="button" data-approve-instrument="${esc(instrumentId)}">Approve Instrument</button><span data-instrument-review-result style="color:#d6a92f;font-size:12px"></span></div></article>`;
  }

  async function render(workspace) {
    const version = nextVersion(workspace);
    removeLegacyApprovalPanel(workspace);
    if (!workspace || !active(workspace) || !pendingTab(workspace)) { clear(workspace); return; }
  ...
  function bind(workspace, panel) {
    panel.querySelectorAll('[data-approve-instrument]').forEach((button) => button.addEventListener('click', async () => {
      ...
      if (!instrumentId || !confirm(`Approve SRA instrument ${instrumentId} for the next lifecycle stage?`)) return;
