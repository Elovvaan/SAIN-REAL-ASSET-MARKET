(() => {
  ...
  function linkageStatus(instrument) {
    if (!instrument.instrumentApproved) return { label:'SOURCE LINKED · INSTRUMENT REVIEW REQUIRED', next:'Pending Review', copy:'The Coin Position source lineage is already attached. Approve the SRA instrument before representation approval can begin.' };
    if (!instrument.representationApproved) return { label:'INSTRUMENT APPROVED · REPRESENTATION APPROVAL REQUIRED', next:'Approval', copy:'Instrument approval is complete. Representation approval is the next handoff before linkage registration and on-chain preparation.' };
    if (!instrument.linkageRegistered) return { label:instrument.sourceLinked?'SOURCE LINKED · LINKAGE CONFIRMATION REQUIRED':'READY TO LINK', next:null, copy:instrument.sourceLinked?'The propagated Coin Position is already the source. Confirm the governed linkage registration to complete the Coin Position handoff.':'Select the authorized source Coin Position and register the linkage.' };
    return { label:'LINKAGE VERIFIED · READY FOR ON-CHAIN', next:'On-Chain', copy:'Coin Position lineage and governed linkage registration are complete. Continue to network readiness and asset identity.' };
  }
  ...
  function linkageMarkup(data) {
    ...
    if(!instruments.length)return '<header><strong>Instrument Linkage</strong><em>LIFECYCLE</em></header><p style="color:#9a9a9a;margin:14px 0 0">No Coin Position instruments are currently in the linkage lifecycle.</p>';
    return `<header><strong>Instrument Linkage</strong><em>LIFECYCLE</em></header><p style="color:#9a9a9a;line-height:1.5">Coin Position → SRA instrument → instrument approval → representation approval → governed linkage → on-chain preparation. Existing source lineage stays visible at every stage.</p><div class="admin-record-list" style="margin-top:14px">${instruments.map(instrument=>{
      const linked=instrument.coinPositionId;
      const status=linkageStatus(instrument);
      const options=selectable.filter(position=>!position.instrumentId||position.instrumentId===instrument.instrumentId).map(position=>`<option value="${esc(position.coinPositionId)}" ${linked===position.coinPositionId?'selected':''}>${esc(position.coinPositionId)} · ${esc(qty(position.availableQuantity))} SRA available · ${esc(position.ownerId||'owner recorded')}</option>`).join('');
      const navigation=status.next?`<button type="button" data-open-instrument-stage="${esc(status.next)}">Continue in Instruments → ${esc(status.next)}</button>`:'';
      const confirmSource=instrument.representationApproved&&instrument.sourceLinked&&!instrument.linkageRegistered?`<button type="button" data-link-instrument="${esc(instrument.instrumentId)}" data-fixed-position="${esc(linked)}">Confirm Source Linkage</button>`:'';
      const manual=!instrument.sourceLinked&&instrument.representationApproved&&!instrument.linkageRegistered?`<div class="admin-record-grid" style="margin-top:12px"><label><span>Eligible source Coin Position</span><select data-link-position>${options||'<option value="">No eligible Coin Position available</option>'}</select></label></div><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button data-link-instrument="${esc(instrument.instrumentId)}" ${options?'':'disabled'}>Link Coin Position</button><span data-link-result style="color:#d6a92f;font-size:12px"></span></div>`:'';
      return `<article class="admin-record-card" style="margin:0" data-linkage-card="${esc(instrument.instrumentId)}"><header><strong>${esc(instrument.instrumentId)}</strong><em>${esc(status.label)}</em></header><div class="admin-record-grid"><div><span>Instrument state</span><strong>${esc(instrument.state)}</strong></div><div><span>Instrument type</span><strong>${esc(instrument.instrumentType)}</strong></div><div><span>Authorized quantity</span><strong>${esc(qty(instrument.authorizedQuantity))} SRA</strong></div><div><span>Source Coin Position</span><strong>${esc(linked||'Not yet registered')}</strong></div><div><span>Instrument approval</span><strong>${instrument.instrumentApproved?'COMPLETE':'REQUIRED'}</strong></div><div><span>Representation approval</span><strong>${instrument.representationApproved?'COMPLETE':'REQUIRED'}</strong></div><div><span>Linkage registration</span><strong>${instrument.linkageRegistered?'COMPLETE':instrument.sourceLinked?'SOURCE PRESENT':'REQUIRED'}</strong></div><div><span>Next stage</span><strong>${esc(instrument.nextStage||'—')}</strong></div></div><p style="color:#9a9a9a;margin:12px 0 0">${esc(status.copy)}</p><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px">${navigation}${confirmSource}<span data-link-result style="color:#d6a92f;font-size:12px"></span></div>${manual}</article>`;
    }).join('')}</div>`;
  }
  ...
  function openInstrumentStage(tab) {
    const workspaceButton=document.querySelector('[data-admin-workspace="instruments"]');
    workspaceButton?.click();
    setTimeout(()=>{
      const workspace=document.querySelector('[data-workspace="instruments"]');
      const tabButton=workspace?.querySelector(`[data-admin-tab="${CSS.escape(tab)}"]`);
      tabButton?.click();
    },0);
  }
  ...
  async function refresh(workspace) {
    ...
    const work=(async()=>{try {
      ...
      node.querySelectorAll('[data-link-instrument]').forEach(button=>button.addEventListener('click',async()=>{
        const row=button.closest('[data-linkage-card]');const coinPositionId=button.dataset.fixedPosition||row?.querySelector('[data-link-position]')?.value;const result=row?.querySelector('[data-link-result]');
