(() => {
  ...
  function currentSupplyMarkup(data) {
    const s=data.supply||{}, r=data.reconciliation||{}, m=data.market||{};
    return `<header><strong>Current SRA Supply</strong><em>STAGE 1</em></header><div style="display:grid;grid-template-columns:repeat(8,minmax(0,1fr));gap:10px;margin-top:12px">${card('Active supply',`${qty(s.activeSra)} SRA`,'independent representation only')}${card('Living market',`${qty(m.liveSra)} SRA`,`${numberText(m.positionCount)} automatic market position${num(m.positionCount)===1?'':'s'}`)}${card('Available',`${qty(s.availableSra)} SRA`,'includes segmented slices')}${card('Reserved',`${qty(s.reservedSra)} SRA`)}${card('Externalized',`${qty(s.externalizedSra)} SRA`)}${card('Retired',`${qty(s.retiredSra)} SRA`)}${card('Recognized value',usd(s.recognizedUsd),'USD representation basis')}${card('Account issuance',`${qty(s.accountIssuedSra)} SRA`,'Coin Account aggregate')}</div><p style="color:#9a9a9a;margin:14px 0 0">Direct-market Coin Positions enter the SRA living market automatically. Segmentation creates position slices, not new SRA issuance. ${numberText(r.derivativePositionCount)} derivative position${num(r.derivativePositionCount)===1?' is':'s are'} excluded from represented-supply issuance.</p>${rowsMarkup(data.sample||[])}`;
  }
  ...
  function linkageStatus(instrument) {
    ...
    if (!instrument.linkageRegistered) return { label:instrument.sourceLinked?'SOURCE LINKED · REGISTRATION IN PROGRESS':'SOURCE LINEAGE PENDING', next:null, copy:instrument.sourceLinked?'The existing source relationship is being registered automatically.':'This instrument will continue when its originating workflow records its Coin Position relationship.' };
  ...
  function linkageMarkup(data) {
    const instruments=data.instruments||[], direct=data.directMarket||{};
    ...
    return `<header><strong>Instrument Linkage</strong><em>LIFECYCLE</em></header><div style="margin-top:12px">${card('Direct to living market',`${qty(direct.liveSra)} SRA`,`${numberText(direct.positionCount)} position${num(direct.positionCount)===1?'':'s'} · automatic`)}</div><p style="color:#9a9a9a;line-height:1.5">Direct-market Coin Positions stay in the SRA living market and remain separate from instrument linkage. Instruments move through their own source lineage and mechanical linkage is registered automatically after representation approval.</p><div class="admin-record-list" style="margin-top:14px">${instruments.map(instrument=>{
      ...
      return `<article class="admin-record-card" style="margin:0" data-linkage-card="${esc(instrument.instrumentId)}"><header><strong>${esc(instrument.instrumentId)}</strong><em>${esc(status.label)}</em></header><div class="admin-record-grid"><div><span>Instrument state</span><strong>${esc(instrument.state)}</strong></div><div><span>Instrument type</span><strong>${esc(instrument.instrumentType)}</strong></div><div><span>Authorized quantity</span><strong>${esc(qty(instrument.authorizedQuantity))} SRA</strong></div><div><span>Source Coin Position</span><strong>${esc(linked||'Awaiting originating workflow')}</strong></div><div><span>Instrument approval</span><strong>${instrument.instrumentApproved?'COMPLETE':'REQUIRED'}</strong></div><div><span>Representation approval</span><strong>${instrument.representationApproved?'COMPLETE':'REQUIRED'}</strong></div><div><span>Linkage registration</span><strong>${instrument.linkageRegistered?'COMPLETE':instrument.sourceLinked?'AUTOMATIC':'WAITING FOR SOURCE'}</strong></div><div><span>Next stage</span><strong>${esc(instrument.nextStage||'—')}</strong></div></div><p style="color:#9a9a9a;margin:12px 0 0">${esc(status.copy)}</p><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px">${navigation}</div></article>`;
    }).join('')}</div>`;
  }
