(() => {
  ...
  async function detailForClosing(closing) {
    if (!closing?.closingId) return null;
    return request(`/api/financing-closing/closings/${encodeURIComponent(closing.closingId)}`);
  }
  ...
  async function load() {
    ...
    try {
      ...
      const cards = opportunities.map((opportunity) => {
        const closing = closingForOpportunity(closings, opportunity.opportunityId);
        return actionCard(opportunity, closing ? closingDetails.get(closing.closingId) : null, authorizations.get(opportunity.opportunityId));
      });
  ...
  async function act(button) {
    ...
    if (result) { result.textContent = 'Processing…'; result.classList.remove('financing-awaiting-error'); }
    try {
      if (button.matches('[data-record-financing-authorization]')) {
        await request(`/api/financing-closing/authorizations/opportunities/${encodeURIComponent(opportunityId)}/approve`, { method: 'POST', body: JSON.stringify({ approval: 'APPROVE', amount: Number(button.dataset.amount) }) });
      } else if (button.matches('[data-open-financing-closing]')) {
        ...
        const closingId = button.dataset.addFinancingCondition;
        const input = card.querySelector(`[data-new-closing-condition="${CSS.escape(closingId)}"]`);
        ...
        const closingId = button.dataset.closingId;
        const conditionId = button.dataset.satisfyFinancingCondition || button.dataset.waiveFinancingCondition;
        const status = button.matches('[data-waive-financing-condition]') ? 'WAIVED' : 'SATISFIED';
