(() => {
  ...
  function renderWallets(data) {
    ...
    return card('Treasury Wallets & Stellar USDC','SEPARATE TREASURY OPERATION',`<div class="admin-record-grid">${field('Wallets',String(wallets.length))}${field('Crypto activity records',String(activity.length))}<div data-stellar-usdc-balance><span>Stellar USDC balance</span><strong>Reading independently…</strong></div><div data-stellar-sep24-status><span>SEP-24 anchor</span><strong>Reading independently…</strong></div></div><p style="color:#9a9a9a;margin:12px 0">This treasury workflow acquires and verifies Circle-issued USDC for treasury inventory. It is separate from direct SRA Coin issuance and on-chain settlement.</p><form data-usdc-conversion-form><div class="admin-record-grid"><label><span>Treasury profile</span><select name="profileId" required><option value="">Select profile</option>${profileOptions}</select></label><label><span>USD amount</span><input name="amount" type="number" min="0.01" step="0.01" required></label><label><span>Provider</span><select name="provider" required><option value="CONFIGURED_ANCHOR">Configured SEP-24 anchor</option><option value="MANUAL_PROVIDER">Approved external provider</option></select></label></div><label style="display:block;margin:12px 0"><input name="confirmLiveConversion" type="checkbox" required> I authorize a live USD-to-USDC treasury conversion request.</label><button type="submit" ${profileOptions?'':'disabled'}>Authorize Treasury Conversion</button><span data-usdc-conversion-result style="color:#d6a92f;font-size:12px;margin-left:10px"></span></form><div style="margin-top:16px">${records}</div>${cctp}`);
  }
