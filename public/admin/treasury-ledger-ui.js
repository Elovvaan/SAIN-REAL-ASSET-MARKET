(() => {
  ...
  function ensurePanel() {
    ...
    anchor.insertAdjacentHTML('afterend', `<section id="sra-treasury-ledger" class="card treasury-shell">
      ...
      <section class="treasury-box funding-instrument-box"><h3>Deposit Platform Commercial Instrument</h3><p style="color:#aaa">Deposit the platform's commercial instrument into SRA, recognize its USD face value, and establish instrument-backed financing capacity. This is not owner-contributed capital.</p>
        ...
        <div class="treasury-actions"><button id="funding-instrument-preview">Preview Instrument Deposit</button><button id="funding-instrument-approve" class="primary" disabled>Deposit & Establish USD Position</button></div>
        <div id="funding-instrument-message" class="treasury-rule">Commercial instrument → SRA deposit → recognized USD → Treasury financing capacity → governed loan financing.</div>
      </section>
    ...
    document.querySelector('#funding-instrument-preview').addEventListener('click', () => void previewFundingInstrument());
    document.querySelector('#funding-instrument-approve').addEventListener('click', () => void approveFundingInstrument());
  }

  function fundingInstrumentInput() {
    return { instrumentId: document.querySelector('#funding-instrument-id')?.value || '', faceValueUsd: Number(document.querySelector('#funding-instrument-value')?.value || 0), termMonths: Number(document.querySelector('#funding-instrument-term')?.value || 36), depositReference: document.querySelector('#funding-instrument-reference')?.value || '' };
  }
  function entryInput() {
    const amount = Number(document.querySelector('#treasury-amount')?.value || 0);
    return { memo: document.querySelector('#treasury-memo')?.value || '', reference: document.querySelector('#treasury-reference')?.value || null, lines: [{ accountId: document.querySelector('#treasury-debit')?.value, side: 'DEBIT', amount, currency: 'USD' }, { accountId: document.querySelector('#treasury-credit')?.value, side: 'CREDIT', amount, currency: 'USD' }] };
  }
  ...
  function render() {
    ...
    state.textContent = treasury.balanced ? 'BALANCED' : 'OUT OF BALANCE'; state.classList.toggle('ok', treasury.balanced);
    document.querySelector('#treasury-kpis').innerHTML = [
      ['Treasury cash / settlement USD', money(treasury.cashBalanceUsd)],
      ['Commercial instrument USD', money(deposits.depositedInstrumentValueUsd)],
      ['Available financing capacity', money(deposits.availableFinancingCapacityUsd)],
      ['SRA represented at par', money(treasury.sraRepresentedAtParUsd)],
      ['Posted journals', number(treasury.journalCount)]
    ...
    const debit = document.querySelector('#treasury-debit'); const credit = document.querySelector('#treasury-credit');
    const oldDebit = debit.value; const oldCredit = credit.value; debit.innerHTML = options; credit.innerHTML = options;
    debit.value = oldDebit || 'TRSY-1000-CASH-USD'; credit.value = oldCredit || 'TRSY-2200-PLATFORM-INSTRUMENT-FUNDING';
    ...
    document.querySelector('#recorded-value-status').innerHTML = `<strong>${number(correction.correctablePositionCount)} legacy position(s) require correction</strong><br>Current represented: ${number(correction.currentRepresentedQuantity)} SRA<br>Recorded-value target: ${number(correction.targetRepresentedQuantity)} SRA`;
    document.querySelector('#correct-recorded-value').disabled = Number(correction.correctablePositionCount || 0) === 0;
  }

  async function load() {
    ensureStyles(); ensurePanel(); if (!document.querySelector('#sra-treasury-ledger')) return;
    try { [treasury, correction] = await Promise.all([request('/api/admin/treasury'), request('/api/admin/recorded-value-representation')]); render(); }
  ...
  async function previewFundingInstrument() {
    const root = document.querySelector('#funding-instrument-message');
    try {
      depositPreview = await request('/api/admin/treasury/funding-instrument-deposits/preview', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(fundingInstrumentInput()) });
      root.className = 'treasury-preview'; root.textContent = `${depositPreview.instrumentName}: ${money(depositPreview.faceValueUsd)} will establish ${money(depositPreview.financingCapacityUsd)} of instrument-backed Treasury financing capacity at 1 SRA = 1 USD.`;
      document.querySelector('#funding-instrument-approve').disabled = false;
    } catch (error) { depositPreview = null; root.className = 'treasury-rule'; root.textContent = error.message; document.querySelector('#funding-instrument-approve').disabled = true; }
  }
  async function approveFundingInstrument() {
    if (!depositPreview || !confirm(`Deposit ${depositPreview.instrumentName} at ${money(depositPreview.faceValueUsd)} into the SRA Platform Treasury?`)) return;
    const root = document.querySelector('#funding-instrument-message');
    try {
      const result = await request('/api/admin/treasury/funding-instrument-deposits/approve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...fundingInstrumentInput(), approval: 'APPROVE' }) });
      root.className = 'treasury-preview'; root.textContent = `Deposit ${result.deposit.transactionId} established ${money(result.deposit.faceValueUsd)} in platform commercial-instrument USD financing capacity.`;
      depositPreview = null; document.querySelector('#funding-instrument-approve').disabled = true; await window.loadSummary?.(); await load();
    } catch (error) { root.className = 'treasury-rule'; root.textContent = error.message; }
  }
  async function previewEntry() {
    const root = document.querySelector('#treasury-entry-preview');
    try { const result = await request('/api/admin/treasury/journals/preview', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(entryInput()) }); root.className = 'treasury-preview'; root.textContent = `Balanced: Debit ${money(result.totalDebits)} / Credit ${money(result.totalCredits)}. ${result.effect}`; document.querySelector('#treasury-post').disabled = false; }
  ...
  async function postEntry() {
    const input = entryInput(); if (!confirm(`Post a balanced treasury entry for ${money(input.lines[0].amount)}?`)) return;
    const root = document.querySelector('#treasury-entry-preview');
    try { const result = await request('/api/admin/treasury/journals/approve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...input, approval: 'APPROVE', idempotencyKey: input.reference || undefined }) }); root.className = 'treasury-preview'; root.textContent = `Journal ${result.journal.entryId} posted with equal debits and credits.`; document.querySelector('#treasury-post').disabled = true; await window.loadSummary?.(); await load(); }
    catch (error) { root.className = 'treasury-message'; root.textContent = error.message; }
  }
  async function correctRecordedValue() {
    const count = Number(correction?.correctablePositionCount || 0); if (!count || !confirm(`Correct ${number(count)} legacy SRA Coin Position(s) to their recognized recorded USD value at par?`)) return;
    const button = document.querySelector('#correct-recorded-value'); button.disabled = true; button.textContent = 'Correcting…';
    try { const result = await request('/api/admin/recorded-value-representation/approve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ approval: 'APPROVE' }) }); window.append?.(`${number(result.correctedPositionCount)} SRA Coin Positions were aligned to recognized recorded USD value.`, 'agent'); await window.loadSummary?.(); await load(); }
  ...
  function initialize() {
    ...
    observer.observe(document.body, { subtree: true, attributes: true, attributeFilter: ['class'] }); setTimeout(() => void load(), 0);
  }
