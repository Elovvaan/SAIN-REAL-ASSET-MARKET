(() => {
  ...
  let booted=false, refreshTimer=null, refreshInFlight=false, refreshAgain=false;
  function loadScript(source,marker){ return new Promise((resolve,reject)=>{ const existing=document.querySelector(`script[${marker}]`); if(existing){ if(existing.dataset.loaded==='true') resolve(); else existing.addEventListener('load',resolve,{once:true}); return; } const script=document.createElement('script'); script.src=source; script.async=false; script.setAttribute(marker,'true'); script.addEventListener('load',()=>{script.dataset.loaded='true';resolve();},{once:true}); script.addEventListener('error',()=>reject(new Error(`Failed to load ${source}`)),{once:true}); document.head.append(script); }); }
  function activeWorkspaceId(){ return document.querySelector('.admin-workspace.active')?.dataset.workspace || location.hash.replace('#admin-','') || 'dashboard'; }
  function requestAdministrationRefresh(source='mutation'){ if(refreshInFlight){refreshAgain=true;return;} clearTimeout(refreshTimer); refreshTimer=setTimeout(async()=>{ refreshInFlight=true; try{ const id=activeWorkspaceId(); document.querySelector(`[data-refresh-workspace="${CSS.escape(id)}"]`)?.click(); await Promise.resolve(window.loadSummary?.()); window.dispatchEvent(new CustomEvent('sra:admin-workspace-synchronized',{detail:{workspaceId:id,source,synchronizedAt:new Date().toISOString()}})); } finally { refreshInFlight=false; if(refreshAgain){refreshAgain=false;requestAdministrationRefresh(source);} } },180); }
  ...
  async function boot(){
    if(booted)return; const admin=document.querySelector('#admin-view:not(.hidden)'); if(!admin)return; booted=true; concealLegacyFirstPaint(admin);
    try{
      const [shellSource,shellMarker]=FEATURES[0]; await loadScript(shellSource,shellMarker); if(!admin.querySelector('.admin-suite')) throw new Error('Administration shell did not mount.'); revealAdminSuite(admin);
      ...
      const treasuryWorkspace=admin.querySelector('[data-workspace="treasury"]'); window.mountAdminTreasuryWorkstation?.(treasuryWorkspace); window.mountAdminTreasuryPresentationOwner?.(treasuryWorkspace);
      window.mountAdminNativePlatformAssetWorkstation?.(admin.querySelector('[data-workspace="native-asset"]')); window.mountAdminFinancialRecordsWorkstation?.(admin.querySelector('[data-workspace="records"]'));
      const coinWorkspace=admin.querySelector('[data-workspace="coin-positions"]'); window.mountAdminCoinRepresentationIntegrityControls?.(coinWorkspace); window.mountAdminCoinLifecycleWorkstation?.(coinWorkspace);
      const marketplaceWorkspace=admin.querySelector('[data-workspace="marketplace"]'); window.mountAdminMarketplaceLifecycleWorkstation?.(marketplaceWorkspace); window.mountAdminMarketplaceStageActions?.(marketplaceWorkspace);
      ...
      window.mountAdminExternalDexAdapter?.(admin); window.mountAdminExternalDexExecutor?.(admin); window.mountAdminSystemHealthWorkstation?.(admin.querySelector('[data-workspace="system"]'));
      window.dispatchEvent(new CustomEvent('sra:admin-booted',{detail:{featureCount:FEATURES.length,bootedAt:new Date().toISOString()}}));
    } catch(error){ booted=false; revealAdminSuite(admin); console.error('SAIN Administration bootstrap failed.',error); }
  }
  ...
  const observer=new MutationObserver(()=>{if(document.querySelector('#admin-view:not(.hidden)')){observer.disconnect();void boot();}}); observer.observe(document.documentElement,{subtree:true,attributes:true,attributeFilter:['class']});
})();
