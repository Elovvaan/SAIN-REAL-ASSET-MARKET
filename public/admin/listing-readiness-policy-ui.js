(() => {
  ...
  function ensurePanel() {
    ...
    anchor.insertAdjacentHTML('afterend', `<section id="listing-readiness-policy-panel" style="margin-top:16px;padding:16px;border:1px solid #493d1f;border-radius:14px;background:#100c05">
      ...
      <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:12px"><label>USD per SRA<input id="policy-unit-price" type="number" value="1" readonly aria-readonly="true"></label><label>Minimum order<input id="policy-minimum-order" type="number" min="0.00000001" step="any" value="1"></label></div>
      <p style="margin:8px 0 0;color:#d6a92f">SRA/USD is fixed at par: 1 SRA = $1.00. This value is not administrator-editable.</p>
  ...
  async function load() {
    ...
    try {
      ...
      document.querySelector('#readiness-policy-summary').textContent = status.active
        ? `Policy active at fixed $1.00 SRA/USD par. ${Number(status.eligibleListingCount || 0).toLocaleString()} listings remain eligible; ${Number(status.readyForPublicationApproval || 0).toLocaleString()} are waiting for publication approval.`
        : `Policy inactive. ${Number(status.eligibleListingCount || 0).toLocaleString()} eligible listings would receive fixed par readiness terms.`;
      const policy = status.policy;
      document.querySelector('#policy-unit-price').value = '1';
      if (policy?.terms) document.querySelector('#policy-minimum-order').value = policy.terms.minimumOrder ?? 1;
  ...
  async function activate() {
    if (!confirm('Activate the standing SRA / USD readiness policy at the fixed par rate of $1.00 per SRA? New eligible listings will receive readiness terms automatically, but will not be published.')) return;
    ...
    window.append?.('Standing SRA / USD readiness policy activated at fixed par. Publication remains separately controlled.', 'agent');
    await load();
  }
