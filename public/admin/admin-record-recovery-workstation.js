(() => {
  ...
  function render(workspace) {
    ...
    p.innerHTML=`<header><strong>Restore Historical Record</strong><em>CONTROLLED RECOVERY</em></header>
      ...
      <label style="display:block;margin-top:14px"><span style="display:block;color:#9a9a9a;font-size:11px;margin-bottom:6px">Existing package / documents</span><input type="file" data-recovery-files multiple accept=".pdf,.png,.jpg,.jpeg,.webp,.doc,.docx,.txt,.csv,.xls,.xlsx"><small style="display:block;color:#777;margin-top:6px">Upload the Financing Availability Letter, funding package, note, or other existing SRA evidence. The system extracts facts for recovery preview; upload alone does not restore a record.</small></label>
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:10px"><button type="button" data-ingest-recovery>Ingest Package</button><span data-ingest-status style="color:#d6a92f;font-size:12px;align-self:center"></span></div>
      <label style="display:block;margin-top:14px"><span style="display:block;color:#9a9a9a;font-size:11px;margin-bottom:6px">Existing record / package facts (JSON) — optional</span><textarea data-record-json rows="10" style="width:100%;background:#090909;color:#eee;border:1px solid #333;border-radius:8px;padding:12px" placeholder='Optional: paste existing JSON if you already have it. Package ingestion can populate extracted facts here.'></textarea></label>
    ...
    const ingestStatus=p.querySelector('[data-ingest-status]');
    p.querySelector('[data-ingest-recovery]').addEventListener('click',async()=>{const files=[...p.querySelector('[data-recovery-files]').files];if(!files.length){ingestStatus.textContent='Choose one or more package documents first.';return;}ingestStatus.textContent='Ingesting and extracting package facts…';const form=new FormData();files.forEach((file)=>form.append('documents',file));try{const r=await fetch('/api/admin/records/recovery-documents',{method:'POST',credentials:'same-origin',body:form});const payload=await r.json().catch(()=>({}));if(!r.ok)throw new Error(payload.error||'Package ingestion failed.');const facts=Array.isArray(payload.extractedFacts)?payload.extractedFacts:[];p.querySelector('[data-record-json]').value=JSON.stringify({sourceDocuments:(payload.records||[]).map((item)=>({documentId:item.document?.id,sha256:item.document?.sha256,originalName:item.document?.originalName,extractionStatus:item.extractionStatus})),extractedFacts:facts},null,2);ingestStatus.textContent=facts.length?'Package ingested. Extracted facts are ready for recovery preview.':'Package stored, but automatic fact extraction is not configured or returned no facts. JSON remains optional for manual facts.';}catch(e){ingestStatus.textContent=e.message;}});
