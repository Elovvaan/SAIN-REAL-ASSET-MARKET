(() => {
  ...
  function onChainCard(item, assets, status, sources, offersByAsset, swapsByAsset) {
    const instrument = item.instrument || {};
    const id = instrumentId(instrument);
    const authorized = authorizedAmount(instrument);
  ...
  async function renderOnChain(workspace, card) {
    ...
    const [approvalStatus, status, assetsResult, sourcesResult, offersResult, swapsResult] = await Promise.all([
