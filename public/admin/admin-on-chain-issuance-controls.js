(() => {
  ...
  function onChainCard(item, assets, status, sources, offersByAsset, swapsByAsset, marketsByAsset, readinessByAsset, nativeMarketsByAsset) {
    ...
    const nativeMarket = asset?.network === 'STELLAR' ? `<section style="margin-top:16px;border-top:1px solid #292929;padding-top:16px"><strong>Step 8 · Activate Funded SRA/XLM Market</strong><p style="color:#9a9a9a;font-size:12px;line-height:1.45">Allocate issued SRA and spendable XLM from the platform distribution account to a genuine two-sided Stellar order book. Stellar minimum reserve and existing liabilities remain unavailable for allocation.</p><div class="admin-record-grid" style="margin-top:10px"><label><span>XLM price per SRA</span><input data-native-market-price type="text" inputmode="decimal" placeholder="XLM per SRA"></label><label><span>SRA sell inventory</span><input data-native-market-sra type="text" inputmode="decimal" placeholder="Amount"></label><label><span>XLM sell inventory</span><input data-native-market-xlm type="text" inputmode="decimal" placeholder="Amount"></label></div><label style="display:flex;gap:8px;align-items:flex-start;color:#cfcfcf;font-size:12px;line-height:1.4;margin-top:10px"><input type="checkbox" data-confirm-native-market style="margin-top:2px"><span>I confirm this submits two live Stellar Mainnet offers using the entered SRA, XLM, and price.</span></label><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button data-activate-native-market="${esc(asset.assetId)}" ${issued ? '' : 'disabled'}>Activate Funded SRA/XLM Market</button><span data-native-market-result style="color:#d6a92f;font-size:12px">${issued ? '' : 'Issue supply first.'}</span></div>${nativeMarketHistory(nativeMarkets)}</section>` : '';
