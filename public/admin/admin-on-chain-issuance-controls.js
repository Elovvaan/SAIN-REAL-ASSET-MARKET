(() => {
  ...
  function lifecycleSteps(item, networkReady, asset, issued, offers = []) {
    ...
    return [
      ...
      step('7 · SRA settlement', issued ? 'READY' : 'WAITING', 'Transfer issued SRA to the recipient wallet and reconcile the network confirmation.'),
      step('Optional exchange', marketState, 'A holder may separately exchange settled SRA through an available network market.'),
    ].join('');
  }
  ...
  function nativeMarketHistory(markets) {
    if (!markets.length) return '<p style="color:#777;font-size:12px;margin:10px 0 0">No funded SRA/XLM market has been activated for this asset.</p>';
    return `<div class="admin-record-list" style="margin-top:10px">${markets.slice(0,5).map((market)=>`<article class="admin-record-card" data-native-market-card="${esc(market.marketId)}" style="margin:0"><header><strong>${esc(market.market)}</strong><em>${esc(market.state)}</em></header><div class="admin-record-grid"><div><span>Reference price</span><strong>${esc(market.referenceXlmPerSra)} XLM / SRA</strong></div><div><span>Bid / ask</span><strong>${esc(market.bidXlmPerSra)} / ${esc(market.askXlmPerSra)}</strong></div><div><span>SRA allocated</span><strong>${esc(market.sraSellAmount)}</strong></div><div><span>XLM allocated</span><strong>${esc(market.xlmSellAmount)}</strong></div><div><span>Spendable XLM</span><strong>${esc(market.xlmAvailable ?? 'Refresh market')}</strong></div><div><span>Stellar reserve</span><strong>${esc(market.minimumReserve ?? market.reserveEvidence?.minimumReserve ?? 'Refresh market')}</strong></div><div><span>Order-book bids</span><strong>${esc(market.bids?.length ?? 'Refresh market')}</strong></div><div><span>Order-book asks</span><strong>${esc(market.asks?.length ?? 'Refresh market')}</strong></div><div><span>Transaction</span><strong>${esc(market.transactionId)}</strong></div></div><div style="margin-top:10px"><button data-reconcile-native-market="${esc(market.marketId)}">Refresh Market</button><span data-native-market-action-result style="color:#d6a92f;font-size:12px;margin-left:8px"></span></div></article>`).join('')}</div>`;
  }

  function onChainCard(item, assets, status, sources, offersByAsset, swapsByAsset, marketsByAsset, readinessByAsset, nativeMarketsByAsset) {
    const instrument = item.instrument || {};
    const id = instrumentId(instrument);
    const authorized = authorizedAmount(instrument);
    ...
    const nativeMarkets = asset ? (nativeMarketsByAsset.get(asset.assetId) || []) : [];
    const usdcReadiness = asset ? readinessByAsset.get(asset.assetId) : null;
    const usdcPreparation = asset?.network === 'STELLAR' ? `<section style="margin-top:16px;border-top:1px solid #292929;padding-top:16px"><strong>Optional Exchange Setup · Stellar USDC</strong><p style="color:#9a9a9a;font-size:12px;line-height:1.45">Prepare the distribution account to hold USDC for an optional SRA/USDC exchange route. Direct SRA issuance and wallet settlement do not use this step.</p><div class="admin-record-grid"><div><span>Trustline</span><strong>${usdcReadiness?.trustline ? 'READY' : 'NOT PREPARED'}</strong></div><div><span>USDC asset</span><strong>${esc(usdcReadiness?.usdcAssetAddress || 'Network-authoritative USDC')}</strong></div><div><span>Recorded USDC balance</span><strong>${esc(usdcReadiness?.usdcBalance ?? 'Refresh after preparation')}</strong></div></div><label style="display:block;margin:10px 0"><input type="checkbox" data-confirm-usdc-trustline> I confirm preparation of the optional Stellar USDC exchange trustline.</label><button data-prepare-usdc-market="${esc(asset.assetId)}">${usdcReadiness?.trustline ? 'Refresh Exchange Readiness' : 'Prepare Optional USDC Exchange'}</button><span data-usdc-prepare-result style="color:#d6a92f;font-size:12px;margin-left:8px"></span></section>` : '';
    ...
    const nativeMarket = asset?.network === 'STELLAR' ? `<section style="margin-top:16px;border-top:1px solid #292929;padding-top:16px"><strong>Optional Post-Settlement Exchange · SRA / XLM Market</strong><p style="color:#9a9a9a;font-size:12px;line-height:1.45">Allocate issued SRA and spendable XLM to a separate two-sided Stellar order book for optional holder exchange after SRA settlement.</p><div class="admin-record-grid" style="margin-top:10px"><label><span>XLM price per SRA</span><input data-native-market-price type="text" inputmode="decimal" placeholder="XLM per SRA"></label><label><span>SRA exchange inventory</span><input data-native-market-sra type="text" inputmode="decimal" placeholder="Amount"></label><label><span>XLM exchange inventory</span><input data-native-market-xlm type="text" inputmode="decimal" placeholder="Amount"></label></div><label style="display:flex;gap:8px;align-items:flex-start;color:#cfcfcf;font-size:12px;line-height:1.4;margin-top:10px"><input type="checkbox" data-confirm-native-market style="margin-top:2px"><span>I confirm this optional exchange action submits two live Stellar Mainnet offers using the entered SRA, XLM, and price.</span></label><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button data-activate-native-market="${esc(asset.assetId)}" ${issued ? '' : 'disabled'}>Activate Optional SRA/XLM Market</button><span data-native-market-result style="color:#d6a92f;font-size:12px">${issued ? '' : 'Issue supply first.'}</span></div>${nativeMarketHistory(nativeMarkets)}</section>` : '';
    ...
    const usdcConversion = asset.network === 'STELLAR' ? `<section style="margin-top:16px;border-top:1px solid #292929;padding-top:16px"><strong>Optional Post-Settlement Exchange · SRA / USDC Market</strong><p style="color:#9a9a9a;font-size:12px;line-height:1.45">Create a separate Stellar order book for holders choosing to exchange SRA and USDC after issuance or settlement. This market does not establish SRA Coin’s fungibility, backing, or settlement status.</p><div class="admin-record-grid" style="margin-top:10px"><label><span>USDC price per SRA</span><input data-usdc-market-price type="text" inputmode="decimal" placeholder="1.0000000"></label><label><span>SRA exchange inventory</span><input data-usdc-market-sra type="text" inputmode="decimal" placeholder="Amount"></label><label><span>USDC exchange inventory</span><input data-usdc-market-usdc type="text" inputmode="decimal" placeholder="Amount"></label></div><label style="display:flex;gap:8px;align-items:flex-start;color:#cfcfcf;font-size:12px;line-height:1.4;margin-top:10px"><input type="checkbox" data-confirm-usdc-market style="margin-top:2px"><span>I confirm this optional exchange action submits two live Stellar Mainnet offers using the entered SRA, USDC, and price.</span></label><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button data-activate-usdc-market="${esc(asset.assetId)}" ${issued ? '' : 'disabled'}>Activate Optional SRA/USDC Market</button><span data-usdc-market-result style="color:#d6a92f;font-size:12px">${issued ? '' : 'Issue supply first.'}</span></div>${usdcMarketHistory(usdcMarkets)}</section><section style="margin-top:16px;border-top:1px solid #292929;padding-top:16px"><strong>Optional Holder Exchange · SRA to USDC</strong><p style="color:#9a9a9a;font-size:12px;line-height:1.45">Quote and execute a separate Stellar order-book exchange from issued SRA into USDC after SRA settlement.</p><div class="admin-record-grid" style="margin-top:10px"><label><span>SRA amount to exchange</span><input data-usdc-swap-sell type="text" inputmode="decimal" autocomplete="off" placeholder="Amount"></label><label><span>Maximum slippage</span><select data-usdc-swap-slippage><option value="50">0.50%</option><option value="100" selected>1.00%</option><option value="250">2.50%</option><option value="500">5.00%</option></select></label><div><span>Live exchange quote</span><strong data-usdc-swap-quote>Request a quote</strong></div></div><label style="display:flex;gap:8px;align-items:flex-start;color:#cfcfcf;font-size:12px;line-height:1.4;margin-top:10px"><input type="checkbox" data-confirm-usdc-swap style="margin-top:2px"><span>I confirm this optional exchange will sell the entered SRA amount for USDC at no less than the quoted minimum.</span></label><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button data-quote-usdc-swap="${esc(asset.assetId)}" ${issued ? '' : 'disabled'}>Quote Optional SRA/USDC Exchange</button><button data-execute-usdc-swap="${esc(asset.assetId)}" disabled>Execute Optional Exchange</button><span data-usdc-swap-result style="color:#d6a92f;font-size:12px">${issued ? '' : 'Issue supply first.'}</span></div>${swapHistory(swaps)}</section>` : '';
    return `<article class="admin-record-card" data-asset-card="${esc(asset.assetId)}"><header><strong>${esc(id)}</strong><em>${esc(marketState || (issued ? 'STEP 7 · TRANSFER / STEP 8 · MARKET' : 'STEP 6 · ISSUE SUPPLY'))}</em></header><div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:12px 0">${lifecycle}</div><div class="admin-record-grid"><div><span>Network</span><strong>${esc(asset.network)}</strong></div><div><span>Asset address</span><strong>${esc(asset.assetAddress)}</strong></div><div><span>Network decimals</span><strong>${esc(asset.decimals)}</strong></div><div><span>Issued supply</span><strong>${esc(asset.issuedSupply ?? '0')}</strong></div><div><span>Asset identity transaction</span><strong>${esc(asset.createdTransactionId || 'Not applicable / not broadcast')}</strong></div><div><span>Last issue transaction</span><strong>${esc(asset.lastIssueTransactionId || '—')}</strong></div></div>
      ...
      <section style="margin-top:16px;border-top:1px solid #292929;padding-top:16px"><strong>Step 7 · Settle SRA On Chain</strong><p style="color:#9a9a9a;font-size:12px;line-height:1.45">Transfer issued SRA from the platform distribution account to the recipient’s wallet. Network confirmation completes the direct on-chain SRA settlement while the Coin Position retains its obligation, documentation, and lineage.</p><div class="admin-record-grid" style="margin-top:10px"><label><span>Settlement amount</span><input data-transfer-amount type="text" inputmode="decimal" autocomplete="off" placeholder="Amount"></label><label><span>Recipient wallet</span><input data-transfer-destination type="text" autocomplete="off" placeholder="Destination wallet"></label></div><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button data-transfer-asset="${esc(asset.assetId)}" data-transfer-symbol="${esc(asset.asset)}" data-transfer-network="${esc(asset.network)}" ${issued ? '' : 'disabled'}>Settle SRA On Chain</button><span data-transfer-result style="color:#d6a92f;font-size:12px">${issued ? '' : 'Issue supply first.'}</span></div></section>
      ${nativeMarket}${usdcPreparation}${usdcConversion}<section style="margin-top:16px;border-top:1px solid #292929;padding-top:16px"><strong>Optional Post-Settlement Exchange · ${esc(asset.network)}</strong><p style="color:#9a9a9a;font-size:12px;line-height:1.45">Submit a separate Mainnet offer for a holder choosing to exchange issued SRA for native ${esc(nativeAsset)} after settlement.</p><div class="admin-record-grid" style="margin-top:10px"><label><span>SRA amount to exchange</span><input data-offer-sell type="text" inputmode="decimal" autocomplete="off" placeholder="Amount"></label><label><span>${esc(nativeAsset)} requested</span><input data-offer-buy type="text" inputmode="decimal" autocomplete="off" placeholder="${esc(nativeAsset)} amount"></label><div><span>Implied ${esc(nativeAsset)} per SRA</span><strong data-offer-rate>—</strong></div></div><label style="display:flex;gap:8px;align-items:flex-start;color:#cfcfcf;font-size:12px;line-height:1.4;margin-top:10px"><input type="checkbox" data-confirm-market-offer style="margin-top:2px"><span>I confirm this optional exchange action submits a live ${esc(asset.network)} Mainnet offer at the entered amounts.</span></label><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button data-market-offer="${esc(asset.assetId)}" data-market-network="${esc(asset.network)}" data-market-native="${esc(nativeAsset)}" ${issued ? '' : 'disabled'}>Submit Optional ${esc(asset.asset)}/${esc(nativeAsset)} Offer</button><span data-market-result style="color:#d6a92f;font-size:12px">${issued ? '' : 'Issue supply first.'}</span></div>${offerHistory(offers,nativeAsset,asset.network)}</section>
    </article>`;
  }
  ...
  function bindOnChain(workspace, card) {
    ...
    card.querySelectorAll('[data-activate-native-market]').forEach((button)=>button.addEventListener('click',async()=>{
      const row=button.closest('[data-asset-card]');const result=row?.querySelector('[data-native-market-result]');
      const xlmPerSra=row?.querySelector('[data-native-market-price]')?.value?.trim();const sraSellAmount=row?.querySelector('[data-native-market-sra]')?.value?.trim();const xlmSellAmount=row?.querySelector('[data-native-market-xlm]')?.value?.trim();
      if(![xlmPerSra,sraSellAmount,xlmSellAmount].every((value)=>Number(value)>0)){if(result)result.textContent='Enter a positive price and both inventory allocations.';return;}
      if(!row?.querySelector('[data-confirm-native-market]')?.checked){if(result)result.textContent='Confirm the live funded market activation first.';return;}
      if(!confirm(`Activate the SRA/XLM market around ${xlmPerSra} XLM per SRA with a 1% bid/ask spread, using ${sraSellAmount} SRA and ${xlmSellAmount} XLM?`))return;
      button.disabled=true;if(result)result.textContent='Validating spendable balances and submitting both Stellar offers…';
      try{const response=await request(`/api/on-chain/assets/${encodeURIComponent(button.dataset.activateNativeMarket)}/markets/native/activate`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({xlmPerSra,sraSellAmount,xlmSellAmount,spreadBps:100,confirmMarketActivation:true})});if(result)result.textContent=`ACTIVE · ${response.transactionId}`;await render(workspace);}
      catch(error){if(result)result.textContent=error.message;button.disabled=false;}
    }));

    card.querySelectorAll('[data-reconcile-native-market]').forEach((button)=>button.addEventListener('click',async()=>{
      const assetRow=button.closest('[data-asset-card]');const marketRow=button.closest('[data-native-market-card]');const result=marketRow?.querySelector('[data-native-market-action-result]');button.disabled=true;if(result)result.textContent='Reading live Stellar SRA/XLM order book and balances…';
      try{const response=await request(`/api/on-chain/assets/${encodeURIComponent(assetRow.dataset.assetCard)}/markets/native/${encodeURIComponent(button.dataset.reconcileNativeMarket)}/reconcile`,{method:'POST'});if(result)result.textContent=`${response.state} · ${response.bids?.length||0} bids / ${response.asks?.length||0} asks`;await render(workspace);}
      catch(error){if(result)result.textContent=error.message;button.disabled=false;}
    }));
