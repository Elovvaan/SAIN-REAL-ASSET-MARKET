(() => {
  ...
  async function render(workspace) {
    ...
    try {
      ...
      const networks = (chainStatus.networks || []).map((item) => item.network).filter(Boolean);
      ...
      card.innerHTML = `<header><strong>On-Chain Issuance</strong><em>${networks.length ? 'READY FOR INSTRUCTION' : 'NO NETWORK'}</em></header>
      ...
      list.innerHTML = approved.map((item) => {
        ...
        const authorized = instrument.authorizedSupply ?? instrument.authorizedAmount ?? instrument.faceValue ?? instrument.faceValueUsd ?? null;
        if (projection) {
          return `<article class="admin-record-card"><header><strong>${esc(id)}</strong><em>ON CHAIN</em></header><div class="admin-record-grid"><div><span>Network</span><strong>${esc(projection.network)}</strong></div><div><span>Mint address</span><strong>${esc(projection.mintAddress)}</strong></div><div><span>Issued supply</span><strong>${esc(projection.issuedSupply)}</strong></div><div><span>Decimals</span><strong>${esc(projection.decimals)}</strong></div><div><span>Issuance transaction</span><strong>${esc(projection.issuanceTransactionId || '—')}</strong></div></div></article>`;
        }
        return `<article class="admin-record-card" data-on-chain-issue-card="${esc(id)}"><header><strong>${esc(id)}</strong><em>APPROVED TO ISSUE</em></header><div class="admin-record-grid"><div><span>Authorized supply / amount</span><strong>${esc(authorized ?? '—')}</strong></div><label><span>Network</span><select data-issue-network>${networks.map((name) => `<option value="${esc(name)}">${esc(name)}</option>`).join('')}</select></label><label><span>Amount to issue</span><input data-issue-amount type="number" min="0.000000001" step="any" placeholder="Amount" required></label><label><span>Decimals</span><input data-issue-decimals type="number" min="0" max="255" step="1" placeholder="Token decimals" required></label></div><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px"><button data-issue-on-chain="${esc(id)}" ${networks.length ? '' : 'disabled'}>Issue On Chain</button><span data-issue-result style="color:#d6a92f;font-size:12px"></span></div></article>`;
      }).join('');
      ...
      card.querySelectorAll('[data-issue-on-chain]').forEach((button) => button.addEventListener('click', async () => {
        ...
        const amount = row?.querySelector('[data-issue-amount]')?.value;
