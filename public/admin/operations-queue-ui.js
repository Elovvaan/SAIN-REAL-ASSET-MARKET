(() => {
  ...
  function renderCoinAgent(agent, actionImpact = null) {
    ...
    if (!root) return;
    if (!agent) {
      root.innerHTML = '<div class="ops-empty">No Coin Agent explanation is available for that position.</div>';
      return;
    }
    root.innerHTML = `<div class="coin-agent-grid">
      ...
      <div><span>Quantity</span><strong>${number(agent.quantity)} ${esc(agent.denomination || 'SRA')}</strong></div>
      <div><span>Available</span><strong>${number(agent.availableQuantity)} ${esc(agent.denomination || 'SRA')}</strong></div>
  ...
  function render(data) {
    ...
    document.querySelectorAll('[data-ops-index]').forEach((element) => element.addEventListener('click', () => {
      ...
      if (positionId) {
        document.querySelector('#coin-agent-search').value = positionId;
        void findCoinAgent();
      } else renderCoinAgent(null);
    }));
