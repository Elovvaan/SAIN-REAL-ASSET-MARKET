(() => {
  ...
  function summaryCard(data, title, override = null) {
    ...
    const countItems = diagnosticCounts ? [
      ['Financial records', diagnosticCounts.FINANCIAL_RECORD], ['Coin Positions', diagnosticCounts.COIN_POSITION],
      ['Instruments', diagnosticCounts.SRA_INSTRUMENT], ['Market listings', diagnosticCounts.MARKETPLACE_LISTING],
      ['Settlements', diagnosticCounts.SRA_SETTLEMENT_RECORD], ['Treasury exceptions', diagnosticCounts.PLATFORM_TREASURY_EXCEPTION]
    ] : [];
  ...
  function conversationMarkup() {
    ...
    return `<section class="admin-record-card" data-agent-operation-card data-agent-conversation><header><strong>SAIN Administrative Agent</strong><em>CONVERSATION</em></header><div style="display:flex;gap:8px;flex-wrap:wrap;margin:12px 0"><button type="button" data-agent-quick-question="Diagnose the platform and tell me what is sticking or failing.">Diagnose platform</button><button type="button" data-agent-quick-question="Where are the instruments in the current flow, and are any missing from the next stage?">Trace instruments</button><button type="button" data-agent-quick-question="What currently needs my approval?">Approval review</button><button type="button" data-agent-quick-question="Give me the operational brief and work queue.">Platform status</button></div><div data-agent-conversation-log style="display:grid;gap:10px;margin:12px 0">${messages}</div><form data-agent-conversation-form><textarea name="question" rows="3" placeholder="Talk with SAIN about what is happening anywhere in the platform..." required style="width:100%;box-sizing:border-box;background:#050505;border:1px solid #292929;border-radius:10px;color:#f5f5f5;padding:12px;resize:vertical"></textarea><div style="display:flex;gap:12px;align-items:center;margin-top:10px"><button type="submit">Send to SAIN</button><span data-agent-conversation-result style="font-size:12px;color:#d6a92f"></span></div></form></section>`;
  }
