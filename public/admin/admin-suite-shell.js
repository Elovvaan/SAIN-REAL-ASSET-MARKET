(() => {
  ...
  const body=id=>document.querySelector(`[data-workspace="${id}"] .admin-workspace-body`);
  ...
  function loadStyle(){if(document.querySelector('link[data-admin-suite]'))return;const link=document.createElement('link');link.rel='stylesheet';link.href=`/admin/admin-suite-shell.css?v=${Date.now()}`;link.dataset.adminSuite='true';document.head.append(link);}
  async function requestJson(url,options={}){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),10000);try{const response=await fetch(url,{...options,cache:'no-store',signal:controller.signal,headers:{Accept:'application/json','Cache-Control':'no-cache',...(options.headers||{})}});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.error||`Request failed with ${response.status}.`);return payload;}catch(error){if(error?.name==='AbortError')throw new Error('The platform did not respond within 10 seconds.');throw error;}finally{clearTimeout(timer);}}

  function makeWorkspace([id,label,description]){const section=document.createElement('section');section.className='admin-workspace';section.dataset.workspace=id;section.dataset.activeTab=TABS[id]?.[0]||'';section.innerHTML=`<div class="admin-workspace-head"><div><p class="admin-eyebrow">SAIN PLATFORM ADMINISTRATION</p><h2>${esc(label)}</h2><p>${esc(description)}</p></div><button type="button" data-refresh-workspace="${id}">Refresh</button></div>${TABS[id]?`<div class="admin-workspace-tabs" role="tablist">${TABS[id].map((tab,index)=>`<button type="button" role="tab" aria-selected="${index===0}" class="${index===0?'active':''}" data-admin-tab="${esc(tab)}">${esc(tab)}</button>`).join('')}</div>`:''}<div class="admin-workspace-body"></div>`;section.addEventListener('click',event=>{const refresh=event.target.closest('[data-refresh-workspace]');if(refresh){void refreshWorkspace(refresh.dataset.refreshWorkspace);return;}const button=event.target.closest('[data-admin-tab]');if(!button)return;section.querySelectorAll('[data-admin-tab]').forEach(item=>{item.classList.remove('active');item.setAttribute('aria-selected','false');});button.classList.add('active');button.setAttribute('aria-selected','true');section.dataset.activeTab=button.dataset.adminTab;renderWorkspace(id);});return section;}
  function dashboardMarkup(){return `<section class="admin-status-section"><div class="admin-section-label">PLATFORM STATUS</div><div class="admin-dashboard-grid">${[['Treasury','treasury'],['Marketplace','marketplace'],['Native Asset','nativeAsset'],['Coin Engine','coinPositions'],['Settlement','settlement'],['System','system'],['Operations','operations']].map(([label,key])=>`<button type="button" class="admin-dashboard-card" data-open-workspace="${key==='nativeAsset'?'native-asset':key==='coinPositions'?'coin-positions':key}"><div class="admin-card-top"><span>${esc(label)}</span><b>→</b></div><strong data-workspace-status="${key}">Checking</strong><small>Current persistent-domain records</small><em>STATUS</em></button>`).join('')}</div></section><section class="admin-command-map"><div class="admin-section-label">PLATFORM COMMAND MAP</div><div class="admin-command-grid">${WORKSPACES.filter(([id])=>id!=='dashboard').map(([id,label,description])=>`<button type="button" data-open-workspace="${id}"><strong>${esc(label)}</strong><span>${esc(description)}</span><b>→</b></button>`).join('')}</div></section>`;}
  ...
  function workspaceRecords(id,tab){const r=state.workspaceData?.records||{};
    if(id==='operations'){
    ...
    if(id==='treasury'){
      if(tab==='Commercial Instruments')return combined(r.instruments.filter(item=>/FUNDING|COMMERCIAL|TREASURY/i.test(JSON.stringify(item))),r.transactions.filter(item=>/PLATFORM_FUNDING_INSTRUMENT_DEPOSIT/i.test(String(item.transactionType||''))));
      if(tab==='Cash Position')return r.ledgerAccounts.filter(item=>/CASH|SETTLEMENT|USD/i.test(JSON.stringify(item)));
      if(tab==='Available Financing'||tab==='Funding Capacity')return combined(r.treasuryProfiles,r.treasuryForecasts,r.instruments.filter(item=>item.financingState||item.treasuryState));
    ...
    if(id==='native-asset'){
      const nativeInstrument=r.instruments.filter(item=>/SRA_PLATFORM_ASSET|NATIVE|PLATFORM/i.test(JSON.stringify(item)));
      if(tab==='Current Asset'||tab==='Approval Status')return nativeInstrument;
      if(tab==='Listing'||tab==='Marketplace Status')return r.marketplaceListings.filter(item=>nativeInstrument.some(ins=>ins.instrumentId===item.instrumentId)||/SRA_PLATFORM_ASSET|NATIVE/i.test(JSON.stringify(item)));
      if(tab==='Export Status')return r.exportPackages.filter(item=>/SRA_PLATFORM_ASSET|NATIVE/i.test(JSON.stringify(item)));
      if(tab==='Ownership')return r.ownershipRecognitions.filter(item=>/SRA_PLATFORM_ASSET|NATIVE/i.test(JSON.stringify(item)));
      ...
      if(tab==='Asset History'||tab==='Publishing'||tab==='Governance')return r.lifecycleEvents.filter(item=>/SRA_PLATFORM_ASSET|NATIVE|PLATFORM_ASSET/i.test(JSON.stringify(item)));
    ...
    if(id==='marketplace'){
      ...
      if(tab==='Orders')return combined(r.marketplaceCommitments,r.transactions.filter(item=>/ORDER/i.test(JSON.stringify(item))));
      if(tab==='Reservations')return combined(r.marketplaceCommitmentWindows,r.marketplaceCommitments.filter(item=>/RESERV/i.test(JSON.stringify(item))));
      ...
      if(tab==='Historical Listings')return combined(r.marketplaceListings,r.lifecycleEvents.filter(item=>/LISTING|MARKETPLACE/i.test(JSON.stringify(item))));
    }
    if(id==='instruments'){
      ...
      if(tab==='History')return combined(r.instruments,r.protectionInstruments,r.lifecycleEvents.filter(item=>/INSTRUMENT/i.test(JSON.stringify(item))));
    ...
    if(id==='settlement'){
      ...
      if(tab==='Destination Verification')return r.settlementInstructions.filter(item=>item.destinationReference||item.receivingAccountReference||item.receivingInstitutionReference);
    ...
    if(id==='connections'){
      if(tab==='Coinbase')return combined(contains(r.settlementAdapters,/COINBASE/i),r.treasuryWallets.filter(item=>/COINBASE/i.test(JSON.stringify(item))),r.enterpriseConnections.filter(item=>/COINBASE/i.test(JSON.stringify(item))));
      if(tab==='FedWire')return combined(r.settlementAdapters.filter(item=>/FEDWIRE|WIRE/i.test(String(item.rail||''))),r.treasuryBankConnections.filter(item=>/FEDWIRE|WIRE/i.test(JSON.stringify(item))));
      if(tab==='ACH')return combined(r.settlementAdapters.filter(item=>String(item.rail||'').toUpperCase()==='ACH'),r.treasuryBankConnections.filter(item=>/ACH/i.test(JSON.stringify(item))));
    ...
    if(id==='users'){
      if(tab==='Administrators')return r.users.filter(item=>JSON.stringify(item.capacities||[]).includes('PLATFORM_ADMIN'));
    ...
    if(id==='system'){
      ...
      if(tab==='Diagnostics')return combined(r.treasuryExceptions,r.outboundEvents.filter(item=>/ERROR|FAILED|EXCEPTION/i.test(recordState(item))),contains(r.lifecycleEvents,/ERROR|FAILED|EXCEPTION|DIAGNOSTIC/i));
    ...
    if(id==='agent'){
      ...
      return r.lifecycleEvents.filter(item=>/AGENT|ADMIN/i.test(JSON.stringify(item)));
    }
    return [];
  }
  ...
  function renderWorkspace(id){const node=body(id);if(!node)return;if(id==='dashboard'){node.innerHTML=dashboardMarkup();syncDashboard();return;}if(state.loading){node.innerHTML=loadingState();return;}if(state.lastError){node.innerHTML=errorState(state.lastError);return;}const section=document.querySelector(`[data-workspace="${id}"]`);const tab=section?.dataset.activeTab||TABS[id]?.[0]||'';const records=workspaceRecords(id,tab);node.innerHTML=recordsMarkup(records,labelFor(id,tab));}
  async function loadWorkspaceData(force=false){if(state.loading&&!force)return state.loading;state.lastError=null;state.loading=requestJson(`/api/admin/workspaces?limit=1000&_=${Date.now()}`).then(data=>{state.workspaceData=data;return data;}).catch(error=>{state.lastError=error.message;throw error;}).finally(()=>{state.loading=null;});return state.loading;}
  async function refreshWorkspace(id){const node=body(id);if(node)node.innerHTML=loadingState();try{await loadWorkspaceData(true);}catch{}renderWorkspace(id);syncDashboard();}
  function syncDashboard(){if(!state.workspaceData)return;for(const [key,status] of Object.entries(state.workspaceData.workspaces||{})){const node=document.querySelector(`[data-workspace-status="${key}"]`);if(node)node.textContent=`${status.state} · ${Number(status.recordCount||0).toLocaleString()}`;}}
  function open(id){if(!WORKSPACES.some(item=>item[0]===id))id='dashboard';document.querySelectorAll('.admin-workspace').forEach(section=>section.classList.toggle('active',section.dataset.workspace===id));document.querySelectorAll('[data-admin-workspace]').forEach(button=>button.classList.toggle('active',button.dataset.adminWorkspace===id));const def=WORKSPACES.find(item=>item[0]===id);document.querySelector('#admin-suite-title').textContent=def[1];document.querySelector('#admin-suite-subtitle').textContent=def[2];const hash=`#admin-${id}`;if(location.hash!==hash)history.replaceState(null,'',hash);renderWorkspace(id);if(!state.workspaceData&&!state.loading)void refreshWorkspace(id);document.querySelector('.admin-suite-main')?.scrollTo({top:0,behavior:'auto'});}
  function mount(){const admin=document.querySelector('#admin-view');if(!admin||state.mounted)return;state.mounted=true;loadStyle();document.body.classList.add('admin-suite-ready');const top=admin.querySelector('.top');const suite=document.createElement('div');suite.className='admin-suite';suite.innerHTML=`<aside class="admin-suite-rail"><div class="admin-suite-brand"><img src="/brand-logo" alt="SRA"><div><strong>SAIN Platform</strong><span>Administration</span></div></div><nav class="admin-suite-nav">${WORKSPACES.map(([id,label],index)=>`<button type="button" data-admin-workspace="${id}" class="${index===0?'active':''}"><strong>${esc(label)}</strong></button>`).join('')}</nav></aside><main class="admin-suite-main"><header class="admin-suite-header"><div><h1 id="admin-suite-title">Dashboard</h1><p id="admin-suite-subtitle">Executive platform status</p></div><div id="admin-suite-account"></div></header><div class="admin-suite-content"></div></main>`;const content=suite.querySelector('.admin-suite-content');WORKSPACES.forEach(def=>content.append(makeWorkspace(def)));admin.insertBefore(suite,admin.firstChild);if(top){suite.querySelector('#admin-suite-account').append(top);top.classList.remove('card');}admin.querySelector('#metrics')?.classList.add('admin-source-metrics');admin.querySelector('.layout')?.classList.add('admin-source-layout');suite.addEventListener('click',event=>{const button=event.target.closest('[data-admin-workspace],[data-open-workspace]');if(button)open(button.dataset.adminWorkspace||button.dataset.openWorkspace);});open(location.hash.replace('#admin-','')||'dashboard');}
