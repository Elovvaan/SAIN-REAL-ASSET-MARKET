(() => {
  const PUBLIC_HOME_VERSION = 'V23_TRANSFERABLE_SETTLEMENT';
  ...
  function statusMarkup() {
    ...
    const stages = Array.isArray(status?.stages) && status.stages.length ? status.stages : [
      { state:'LOADING', label:'Verified Coin Positions', detail:'Reading current position records.' },
      { state:'LOADING', label:'On-chain SRA assets', detail:'Reading current on-chain records.' },
      { state:'LOADING', label:'Transfer and settlement', detail:'Reading current settlement records.' }
    ];
    return `<section class="public-infrastructure-status" aria-label="Current SRA infrastructure status"><div class="public-section-heading"><div><p class="eyebrow">CURRENT PLATFORM STATUS</p><h2>Positions, assets, and settlement</h2></div><span class="public-phase">${escape(stateLabel(status?.phase || 'INFRASTRUCTURE BUILDOUT'))}</span></div><div class="public-status-grid">${stages.map((stage)=>`<article><div><span class="public-status-dot" data-state="${escape(stage.state)}"></span><strong>${escape(stage.label)}</strong></div><em>${escape(stateLabel(stage.state))}</em><p>${escape(stage.detail)}</p></article>`).join('')}</div></section>`;
  }

  function infrastructureNarrative() {
    return `<section class="public-infrastructure-intro"><div class="public-infrastructure-hero"><p class="eyebrow">ASSET · MARKET · SETTLEMENT INFRASTRUCTURE</p><h2>Make productive assets transferable.</h2><p>SRA verifies real economic activity, forms source-backed SRA Coin Positions, and carries those positions into governed markets and settlement.</p><strong class="public-infrastructure-thesis">One fungible SRA Coin. Every position preserves its source, obligation, documentation, ownership, and lineage.</strong><div class="public-infrastructure-actions"><button class="primary-button" data-public-action="signup">Bring an asset</button><button class="secondary-button" data-public-prompt="Show me how an asset moves from verification to a transferable SRA Coin Position and settlement.">See how the system works</button><a class="secondary-button" href="/verify-financing.html">Verify financing authorization</a><a class="secondary-button" href="/exchange-settled-sra.html">Exchange settled SRA</a></div></div><div class="public-model-shift"><p class="eyebrow">THE SRA FLOW</p><div><span>Documented source</span><strong>Verify the position and obligation</strong><small>Economic activity enters through its own evidence, authority, and supporting records.</small></div><div class="sra-model"><span>Fungible SRA Coin</span><strong>Transfer, settle, and continue</strong><small>Confirmed delivery completes settlement. The recipient controls the delivered SRA Coin and its next transfer.</small></div></div></section><section class="public-asset-path" aria-label="How productive assets move through SRA"><article><span>01</span><strong>Bring the asset</strong><p>Connect an eligible productive asset, revenue stream, obligation, or market source with its supporting records.</p></article><article><span>02</span><strong>Verify the position</strong><p>Record its authority, ownership, economic value, obligation, and evidence.</p></article><article><span>03</span><strong>Create SRA Coin</strong><p>Form a source-backed Coin Position in the same fungible SRA Coin and, when authorized, represent it on chain.</p></article><article><span>04</span><strong>Transfer and settle</strong><p>Deliver SRA directly to the recipient. After settlement, the holder may transfer it again or exchange it into another asset through an available market.</p></article></section>${statusMarkup()}<section class="public-ecosystem"><div class="public-section-heading"><div><p class="eyebrow">ONE INFRASTRUCTURE · DISTINCT ROLES</p><h2>Built for the entire asset lifecycle</h2></div></div><div><article><strong>Businesses and asset providers</strong><p>Bring productive activity into verification and source-backed Coin Position formation.</p></article><article><strong>SRA Coin holders</strong><p>Receive settled SRA Coin, control the delivered asset, use platform services, transfer it, or enter an available market.</p></article><article><strong>Markets and institutions</strong><p>Receive transferable SRA, record custody and trading, and support holder-directed exchange into other assets after settlement.</p></article></div></section>`;
  }
  ...
  function buildPublicHome() {
    ...
    if (!document.querySelector('.public-infrastructure-intro')) {
      workspace.insertAdjacentHTML('beforebegin', infrastructureNarrative());
    } else {
      const currentStatus = document.querySelector('.public-infrastructure-status');
      if (currentStatus) currentStatus.outerHTML = statusMarkup();
    }
