...
export function createSaneRouter(service = new SaneSkillService(), edxOperationsService = null, sraAgentService = null) {
  ...
  router.get('/operations-queue', async (_req, res) => {
    if (!unifiedOperationsQueue) return res.status(503).json({ error: 'Unified market operations queue is unavailable.' });
    try { return res.json(await unifiedOperationsQueue.explainPersisted()); }
    catch (error) { return res.status(500).json({ error: error.message, code: 'SRA_OPERATIONS_QUEUE_FAILED' }); }
  });
