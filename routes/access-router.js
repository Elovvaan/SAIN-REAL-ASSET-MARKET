...
function setSessionCookie(res, token) { res.setHeader('Set-Cookie', `sra_session=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=43200`); }
...
export function createAccessRouter(marketplace, service = new AccessService()) {
  ...
  router.get('/session', (req, res) => { const session = service.getSession(readCookie(req, 'sra_session')); res.json({ authenticated: Boolean(session), session }); });
  router.post('/signup', (req, res) => { try { const result = service.signup(req.body); setSessionCookie(res, result.token); res.status(201).json({ authenticated: true, session: result.session }); } catch (error) { res.status(400).json({ error: error.message }); } });
