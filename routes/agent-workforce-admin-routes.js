...
export async function installAgentWorkforceAdminRoutes({ router, domain, database, requireAdmin }) {
  ...
  const queueRates = (queue) => [...(queue.queue || []), ...(queue.exceptions || [])].map(entry => ({
    sourceRecordId: entry.id, stage: entry.stage, state: entry.state, requestedAction: entry.nextAction || null,
    participantId: entry.participantId || null, instrumentId: entry.instrumentId || null, listingId: entry.listingId || null,
    positionId: entry.positionId || null, serviceFee: serviceFees.quoteWorkflowStage(entry.stage),
  }));
  ...
  router.get('/api/admin/agent-workforce/status', async (req, res) => {
    ...
    return res.json({ workforce: workforce.status(), agentOS: agentOS.brief(), platinumPhase6: { service: 'AUTONOMOUS_OPERATIONAL_CONTINUATION', records: domain.list('AUTONOMOUS_CONTINUATION').length, followUps: domain.list('AUTONOMOUS_CONTINUATION_FOLLOW_UP').filter((record) => record.status === 'OPEN').length }, synchronizedAgents: { createdCount: 0, existingCount: registeredAgents.length, agentCount: registeredAgents.length, execution: 'ON_DEMAND' }, initialRun: { createdCount: 0, completedCount: 0, skippedCount: 0, execution: 'ON_DEMAND' }, serviceFeeSchedule: serviceFees.policy(), serviceFeeBilling: serviceFeeBilling.status(), workflowServiceRates: serviceFees.workflowRates(), currentQueueServiceRates: queueRates(queue) });
  });

  router.get('/api/admin/agent-workforce/service-rates', async (req, res) => {
    const session = await requireAdmin(req, res); if (!session) return;
    const queue = await operationsQueue.explainPersisted();
    return res.json({ schedule: serviceFees.policy(), billing: serviceFeeBilling.status(), workflowRates: serviceFees.workflowRates(), currentQueue: { state: queue.state, totalAwaitingAction: queue.totalAwaitingAction, totalExceptions: queue.totalExceptions, records: queueRates(queue) } });
  });

  router.get('/api/admin/agent-workforce/service-fees', async (req, res) => {
    const session = await requireAdmin(req, res); if (!session) return;
    return res.json({ status: serviceFeeBilling.status(), records: serviceFeeBilling.listCharges({ payerId: req.query.payerId, subjectId: req.query.subjectId, state: req.query.state }) });
  });
  ...
  router.post('/api/admin/agent-workforce/run', async (req, res) => {
    ...
    try {
      ...
      return res.json({ agentOS: agentOS.brief(), synchronizedAgents: { createdCount: synchronized.created.length, existingCount: synchronized.existing.length, agentCount: synchronized.agentCount }, queue: { state: queue.state, totalAwaitingAction: queue.totalAwaitingAction, totalExceptions: queue.totalExceptions, nextRecommendedAction: queue.nextRecommendedAction }, serviceRates: queueRates(queue), run, continuation });
  ...
  router.post('/api/admin/agent-workforce/work', async (req, res) => {
    const session = await requireAdmin(req, res); if (!session) return;
    try { const work = await workforce.assignWork(req.body || {}, session); return res.status(201).json({ ...work, serviceFee: serviceFees.quoteWorkOrder(work) }); }
  ...
  router.post('/api/admin/agent-workforce/work/:workOrderId/accept', async (req, res) => {
    ...
    try { serviceFeeBilling.validateServicingTarget({ payerId, servicingAccountId: req.body?.servicingAccountId, dueDate: req.body?.dueDate }); }
    catch (error) { return res.status(422).json({ error: error.message, code: 'SRA_AGENT_SERVICE_FEE_SERVICING_VALIDATION_FAILED' }); }
    let accepted;
    ...
    let feeAssessment;
    try { feeAssessment = await serviceFeeBilling.assessAcceptedWork(accepted.work, { payerId: req.body?.payerId, payerType: req.body?.payerType }, session.id); }
    catch (error) { return res.json({ ...accepted, serviceFee: serviceFees.quoteWorkOrder(accepted.work), serviceFeeAssessment: { assessed: false, error: error.message, code: 'SRA_AGENT_SERVICE_FEE_ASSESSMENT_FAILED' }, servicing: null }); }
    let servicing = null;
    if (feeAssessment.charge && req.body?.servicingAccountId && req.body?.dueDate) {
      try { servicing = await serviceFeeBilling.attachChargeToServicing(feeAssessment.charge.chargeId, { servicingAccountId: req.body.servicingAccountId, dueDate: req.body.dueDate, recurrence: req.body.recurrence || null }, session.id); }
      catch (error) { servicing = { attached: false, error: error.message, code: 'SRA_AGENT_SERVICE_FEE_SERVICING_FAILED' }; }
    }
