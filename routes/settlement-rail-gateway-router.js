...
export function createSettlementRailGatewayRouter(service,bankAchOriginationService=new BankAchOriginationService(),stellarUsdcSettlementService=null,moneyGramSandboxCertificationService=null){
  ...
  router.get('/stellar-usdc/sep24/sandbox-tests',(_req,res)=>{try{if(!moneyGramSandboxCertificationService)throw new Error('MoneyGram sandbox certification service is unavailable.');return res.json(moneyGramSandboxCertificationService.status());}catch(error){return handleError(res,error);}});
  router.post('/stellar-usdc/sep24/sandbox-tests',async(req,res)=>{try{const actor=authenticatedActor(req);if(!actor)return res.status(401).json({error:'Authenticated SRA operations identity is required.'});if(!moneyGramSandboxCertificationService)throw new Error('MoneyGram sandbox certification service is unavailable.');return res.status(201).json(await moneyGramSandboxCertificationService.start(req.body||{},actor));}catch(error){return handleError(res,error);}});
  router.post('/stellar-usdc/sep24/sandbox-tests/:testId/refresh',async(req,res)=>{try{const actor=authenticatedActor(req);if(!actor)return res.status(401).json({error:'Authenticated SRA operations identity is required.'});if(!moneyGramSandboxCertificationService)throw new Error('MoneyGram sandbox certification service is unavailable.');return res.json(await moneyGramSandboxCertificationService.refresh(req.params.testId,actor));}catch(error){return handleError(res,error);}});
  router.get('/stellar-usdc/sep24/sandbox-tests-evidence',(_req,res)=>{try{if(!moneyGramSandboxCertificationService)throw new Error('MoneyGram sandbox certification service is unavailable.');res.setHeader('Content-Disposition','attachment; filename="moneygram-sandbox-evidence.json"');return res.json(moneyGramSandboxCertificationService.evidence());}catch(error){return handleError(res,error);}});
