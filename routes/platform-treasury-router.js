...
export function createPlatformTreasuryRouter(service,usdcConversions=null,cctpTransfers=null){const router=express.Router();
...
router.get('/cctp/status',(_req,res)=>{try{return res.json(cctpTransfers?.status()||{protocol:'CIRCLE_CCTP_V2',configured:false});}catch(e){return fail(res,e);}});
router.get('/cctp/transfers',(req,res)=>res.json({transfers:cctpTransfers?.list({state:req.query.state||null,destinationNetwork:req.query.destinationNetwork||null})||[]}));
router.get('/cctp/transfers/:transferId',(req,res)=>{const item=cctpTransfers?.get(req.params.transferId);return item?res.json(item):res.status(404).json({error:'CCTP transfer not found.'});});
router.post('/cctp/transfers',async(req,res)=>{try{if(!cctpTransfers)throw new Error('Circle CCTP transfer service is unavailable.');return res.status(201).json(await cctpTransfers.authorize(req.body||{},conversionActor(req)));}catch(e){return fail(res,e);}});
router.post('/cctp/transfers/:transferId/burn',async(req,res)=>{try{return res.json(await cctpTransfers.burn(req.params.transferId,req.body||{},conversionActor(req)));}catch(e){return fail(res,e);}});
router.post('/cctp/transfers/:transferId/attest',async(req,res)=>{try{return res.json(await cctpTransfers.attest(req.params.transferId,req.body||{},conversionActor(req)));}catch(e){return fail(res,e);}});
router.post('/cctp/transfers/:transferId/mint',async(req,res)=>{try{return res.json(await cctpTransfers.mint(req.params.transferId,req.body||{},conversionActor(req)));}catch(e){return fail(res,e);}});
router.post('/cctp/transfers/:transferId/reconcile',async(req,res)=>{try{return res.json(await cctpTransfers.reconcile(req.params.transferId,req.body||{},conversionActor(req)));}catch(e){return fail(res,e);}});
return router;}
