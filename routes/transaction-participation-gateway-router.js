...
function failure(res,error){const message=error?.message||String(error);const status=/not found/i.test(message)?404:/access|expired|verified/i.test(message)?403:400;return res.status(status).json({error:message});}
...
export function createTransactionParticipationGatewayRouter(service,database=null){
 ...
 router.post('/receipt',async(req,res)=>{try{return res.json(await service.confirmReceipt(credentials(req),req.body||{}));}catch(error){return failure(res,error);}});
 router.post('/contact',async(req,res)=>{try{return res.json(await service.identifyContact(credentials(req),req.body||{}));}catch(error){return failure(res,error);}});
 router.post('/questions',async(req,res)=>{try{const recorded=await service.askQuestion(credentials(req),req.body||{});const response=await counterparty.resolveParticipationEvent(recorded.event.eventId);return res.json({...recorded,response});}catch(error){return failure(res,error);}});
 router.post('/issues',async(req,res)=>{try{const recorded=await service.reportIssue(credentials(req),req.body||{});const response=await counterparty.resolveParticipationEvent(recorded.event.eventId);return res.json({...recorded,response});}catch(error){return failure(res,error);}});
 router.post('/processing-confirmation',async(req,res)=>{try{return res.json(await service.confirmProcessing(credentials(req),req.body||{}));}catch(error){return failure(res,error);}});
