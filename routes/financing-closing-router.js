...
export function createFinancingClosingRouter(service) {
  ...
  router.get('/letters/opportunities/:opportunityId', (req, res) => {
    try {
      ...
      if (decision !== 'APPROVE' || !['CLOSING', 'READY_TO_FUND', 'FUNDED', 'SERVICING'].includes(stage)) {
        return res.status(409).send('A financing availability letter is available only after an approved credit decision.');
      }
      ...
      const approvedAmount = Number(opportunity.creditDecision?.approvedAmount || financing.amount || opportunity.requestedAmount || 0);
      ...
      const document = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${html(title)}</title><style>
        :root{font-family:Arial,Helvetica,sans-serif;color:#171717;background:#efefef}*{box-sizing:border-box}body{margin:0;padding:32px}.sheet{max-width:850px;margin:0 auto;background:#fff;padding:58px 64px;box-shadow:0 3px 24px rgba(0,0,0,.14)}.brand{display:flex;justify-content:space-between;gap:20px;border-bottom:3px solid #b68b16;padding-bottom:18px}.brand h1{margin:0;font-size:24px}.brand small{display:block;color:#666;margin-top:5px}.date{font-size:13px;text-align:right}.title{font-size:20px;margin:34px 0 22px}.lead{font-size:15px;line-height:1.65}.grid{display:grid;grid-template-columns:1fr 1fr;gap:0;border:1px solid #ddd;margin:26px 0}.grid div{padding:13px 15px;border-bottom:1px solid #e5e5e5}.grid div:nth-last-child(-n+2){border-bottom:0}.grid span{display:block;font-size:10px;color:#666;text-transform:uppercase;letter-spacing:.08em}.grid strong{display:block;margin-top:5px;font-size:14px}.status{font-weight:700;color:#80600d}.notice{margin-top:24px;padding:16px;border-left:4px solid #b68b16;background:#faf7ed;font-size:12px;line-height:1.55;color:#444}.signature{margin-top:42px}.signature strong{display:block}.footer{margin-top:46px;border-top:1px solid #ddd;padding-top:13px;font-size:10px;color:#666;line-height:1.5}.print{max-width:850px;margin:0 auto 12px;text-align:right}.print button{background:#111;color:white;border:0;border-radius:6px;padding:10px 16px;cursor:pointer}@media print{body{background:#fff;padding:0}.sheet{box-shadow:none;max-width:none;padding:40px 50px}.print{display:none}@page{size:letter;margin:.45in}}
      </style></head><body><div class="print"><button onclick="window.print()">Print / Save as PDF</button></div><main class="sheet"><header class="brand"><div><h1>SAIN Platform</h1><small>Financing Operations</small></div><div class="date">${html(dateLabel(issuedAt))}</div></header><h2 class="title">${html(title)}</h2><p class="lead">To whom it may concern:</p><p class="lead">This letter confirms that SAIN Platform records reflect an approved financing commitment for <strong>${html(customerName)}</strong> in an amount up to <strong>${html(usd(approvedAmount))}</strong> for the referenced ${html(purpose.toLowerCase())} transaction. The financing has completed credit decision and has a posted financing authorization in the SAIN financing ledger.</p><section class="grid"><div><span>Applicant / Customer</span><strong>${html(customerName)}</strong></div><div><span>Approved Amount</span><strong>${html(usd(approvedAmount))}</strong></div><div><span>Opportunity Reference</span><strong>${html(opportunityId)}</strong></div><div><span>Financing Authorization</span><strong>${html(reference)}</strong></div><div><span>Financing Status</span><strong class="status">${html(statusLabel)}</strong></div><div><span>Closing Reference</span><strong>${html(closing?.closingId || 'Not yet opened')}</strong></div></section><p class="lead">This letter may be used as evidence of the approved financing recorded for the transaction identified above. Final disbursement remains subject to completion of the applicable closing requirements and funding authorization unless the financing status above is shown as FUNDED.</p><div class="notice"><strong>Important distinction:</strong> This is a financing-availability confirmation generated from SAIN's authoritative financing records. It is not a depository account statement, bank balance verification, or representation that external settlement has already occurred.</div><div class="signature"><strong>SAIN Platform Administration</strong><span>Financing Operations</span></div><div class="footer">Issued ${html(dateLabel(issuedAt))} · Opportunity ${html(opportunityId)} · Financing authorization ${html(reference)} · Generated from current SAIN financing records.</div></main></body></html>`;
