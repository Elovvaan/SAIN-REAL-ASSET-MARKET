...
export function createFinancingClosingRouter(service) {
  ...
  router.post('/closings/:closingId/pre-closing-disbursements/authorize', async (req, res) => { try { return res.status(201).json(await service.authorizePreClosingDisbursement(req.params.closingId, req.body || {}, actorId(req))); } catch (error) { return fail(res, error); } });
