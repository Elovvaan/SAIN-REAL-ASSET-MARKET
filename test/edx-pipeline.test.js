...
test('complete company-controlled EDX pipeline proves private and published outcomes', async () => {
  ...
  const connectorResponse = await client.post('/api/edx/connections/connector-definitions', connectorDefinition).expect(201);
  assert.equal(connectorResponse.body.connectorDefinitionId, connectorDefinition.connectorDefinitionId);

  const connectionResponse = await client.post('/api/edx/connections/connections', connection).expect(201);
  assert.equal(connectionResponse.body.enterpriseId, enterpriseId);
  await client.post(`/api/edx/connections/connections/${connection.connectionId}/transition`, { state: 'AUTHORIZATION_PENDING' }).expect(200);
  await client.post(`/api/edx/connections/connections/${connection.connectionId}/transition`, {
    state: 'CONNECTED',
    authorizationReference: 'TEST-AUTHORIZATION-001'
  }).expect(200);
  await client.post(`/api/edx/connections/connections/${connection.connectionId}/transition`, { state: 'ACTIVE' }).expect(200);

  await client.post('/api/edx/permissions/policies', policy).expect(201);
  await client.post(`/api/edx/permissions/policies/${policy.policyId}/transition`, { state: 'ACTIVE' }).expect(200);

  const extractionRequest = await client.post('/api/edx/extractions/extraction-requests', {
  ...
  await client.post(`/api/edx/extractions/extraction-requests/${extractionRequestId}/transition`, {
    state: 'APPROVED',
    companyApprovalReference: 'TEST-EXTRACTION-APPROVAL-001'
  }).expect(200);

  const extractionExecution = await client.post(`/api/edx/extractions/extraction-requests/${extractionRequestId}/execute`, {
  ...
  const normalized = await client.post(`/api/edx/normalization/extraction-results/${extractionResult.extractionResultId}/normalize`, {
  ...
  const verifiedRecord = await client.post(`/api/edx/normalization/normalized-records/${normalizedRecordId}/verification`, {
    verificationState: 'VERIFIED',
    note: 'Fixture source and structure verified.'
  }).expect(200);
  assert.equal(verifiedRecord.body.verificationState, 'VERIFIED');

  const snapshot = await client.post('/api/edx/snapshots/snapshots/generate', {
    enterpriseId,
  ...
  const valuePackage = await client.post('/api/edx/value-packages/value-packages/generate', {
  ...
  const privateDecision = await client.post('/api/edx/marketplace/publication-decisions', {
  ...
  const projectionsAfterPrivate = await client.get(`/api/edx/marketplace/marketplace-projections?enterpriseId=${enterpriseId}`).expect(200);
  assert.equal(projectionsAfterPrivate.body.marketplaceProjections.length, 0);

  const publishDecision = await client.post('/api/edx/marketplace/publication-decisions', {
    valuePackageId: valuePackage.body.valuePackageId,
    decision: 'PUBLISH_TODAY'
  }).expect(201);
  assert.equal(publishDecision.body.state, 'PENDING');

  await client.post(`/api/edx/marketplace/publication-decisions/${publishDecision.body.publicationDecisionId}/approve`, {
  ...
  const published = await client.post(`/api/edx/marketplace/publication-decisions/${publishDecision.body.publicationDecisionId}/execute`, {}).expect(200);
  ...
  const projectionsAfterPublish = await client.get(`/api/edx/marketplace/marketplace-projections?enterpriseId=${enterpriseId}`).expect(200);
