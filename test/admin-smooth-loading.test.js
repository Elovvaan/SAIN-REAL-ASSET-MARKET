...
test('admin bootstrap mounts the shell independently and isolates parallel feature failures', () => {
  assert.match(bootstrap, /admin-performance-runtime\.js/);
  assert.match(bootstrap, /void ensurePerformanceRuntime\(\)\.catch/);
  assert.doesNotMatch(bootstrap, /await ensurePerformanceRuntime\(\)/);
