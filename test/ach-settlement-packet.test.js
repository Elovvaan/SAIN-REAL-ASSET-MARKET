...
test('financing export package assembles linked deal documents with the ACH settlement completion page', async () => {
  ...
  domain.put('FUNDING_OPPORTUNITY', 'FOR-1', {
    ...
    transactionProfile: {
      purchaserName: 'House Morris Trust', payeeName: 'Example Audi Dealer',
      vehicleYear: '2026', vehicleMake: 'Audi', vehicleModel: 'Q5', vin: 'WA1EXAMPLE1234567',
      agreementNumber: 'PA-7788', purchasePrice: 50000,
      sourceDocumentId: 'DOC-PURCHASE-1', sourceDocumentSha256: 'a'.repeat(64),
    },
  });
  domain.put('FUNDING_OPPORTUNITY_EVIDENCE', 'FOE-1', {
    evidenceId: 'FOE-1', opportunityId: 'FOR-1', documentId: 'DOC-PURCHASE-1', evidenceType: 'PURCHASE_AGREEMENT',
  });
  domain.put('FINANCING_CLOSING', 'FCL-1', {
    closingId: 'FCL-1', opportunityId: 'FOR-1', beneficiaryName: 'Example Audi Dealer',
    conditions: [{ conditionId: 'C-1', title: 'Executed purchase agreement', status: 'SATISFIED' }],
    documentaryEvidence: {
  ...
  const documentRecord = {
    id: 'DOC-PURCHASE-1', originalName: 'Audi Purchase Agreement.pdf', mimeType: 'application/pdf',
    documentType: 'PURCHASE_AGREEMENT', sha256: 'a'.repeat(64), uploadedAt: '2026-08-22T12:00:00.000Z',
    extraction: { status: 'EXTRACTED', facts: { documentType: 'VEHICLE_PURCHASE_AGREEMENT' } },
  };
  const documents = {
    async initialize() {},
  ...
  assert.ok(assembled.getPageCount() >= 3, 'cover + source agreement + settlement page should be present');
});
