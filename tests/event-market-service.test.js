...
test('event market requires governed listing, confirmed execution, evidence resolution, and exact settlement',async()=>{
  ...
  await events.recordSignal(market.eventMarketId,{yesPrice:.68,noPrice:.32,volume:425,score:'24–17',eventClock:'4:18 Q4',eventStatus:'IN_PROGRESS',sourceType:'OFFICIAL_DATA',sourceName:'Official league feed',sourceReference:'FEED-1',observedAt:new Date().toISOString()},{participantId:'IO',capacity:'INSTITUTIONAL_OPERATOR'});
  assert.equal(events.summary(market.eventMarketId).signals.length,2);
  assert.equal(events.summary(market.eventMarketId).contracts.find((item)=>item.outcome==='YES').currentPrice,.68);
